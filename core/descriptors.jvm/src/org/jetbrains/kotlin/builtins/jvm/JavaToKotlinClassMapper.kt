...
object JavaToKotlinClassMapper : PlatformToKotlinClassMapper {
    override fun mapPlatformClass(classDescriptor: ClassDescriptor): Collection<ClassDescriptor> {
        konst className = DescriptorUtils.getFqName(classDescriptor)
    ...
    fun mapPlatformClass(fqName: FqName, builtIns: KotlinBuiltIns): Collection<ClassDescriptor> {
        konst kotlinAnalog = mapJavaToKotlin(fqName, builtIns) ?: return emptySet()

        konst kotlinMutableAnalogFqName = JavaToKotlinClassMap.readOnlyToMutable(kotlinAnalog.fqNameUnsafe) ?: return setOf(kotlinAnalog)
    ...
    fun isMutable(type: KotlinType): Boolean {
        konst classDescriptor = TypeUtils.getClassDescriptor(type)
    ...
    private inline fun convertToOppositeMutability(
        ...
        konst oppositeClassFqName = oppositeNameExtractor(DescriptorUtils.getFqName(descriptor))
