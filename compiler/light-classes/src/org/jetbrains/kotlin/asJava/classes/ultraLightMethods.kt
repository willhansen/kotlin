...
internal abstract class KtUltraLightMethod(
    internal konst delegate: PsiMethod,
    ...
    private class KtUltraLightThrowsReferenceListBuilder(private konst parentMethod: PsiMethod) :
    ...
    protected fun computeThrowsList(methodDescriptor: FunctionDescriptor?): PsiReferenceList {
        konst builder = KtUltraLightThrowsReferenceListBuilder(parentMethod = this)

        if (methodDescriptor !== null) {
            for (ex in FunctionCodegen.getThrownExceptions(methodDescriptor, LanguageVersionSettingsImpl.DEFAULT)) {
                konst psiClassType = ex.defaultType.asPsiType(support, TypeMappingMode.DEFAULT, builder) as? PsiClassType
    ...
    protected fun computeCheckNeedToErasureParametersTypes(methodDescriptor: FunctionDescriptor?): Boolean {
        ...
        konst hasSpecialSignatureInfo = methodDescriptor.getSpecialSignatureInfo()
            ?.let { it.konstueParametersSignature != null } ?: false
        ...
        konst hasStarProjectionParameterType = methodDescriptor.konstueParameters
        ...
        return methodDescriptor.overriddenDescriptors
            ...
            .any { it.konstueParameters.any { parameter -> parameter.type is RawType } }
    }

    abstract override fun buildTypeParameterList(): PsiTypeParameterList

    abstract konst checkNeedToErasureParametersTypes: Boolean

    override konst psiTypeForNullabilityAnnotation: PsiType? get() = returnType

    // These two overrides are necessary because ones from KtLightMethodImpl suppose that clsDelegate.returnTypeElement is konstid
    ...
    private konst _deprecated: Boolean by lazyPub { kotlinOrigin?.isDeprecated(support) ?: false }
...
internal class KtUltraLightMethodForSourceDeclaration(
    ...
    support: KtUltraLightSupport,
    containingClass: KtLightClass,
    private konst forceToSkipNullabilityAnnotation: Boolean = false,
    methodIndex: Int
) : KtUltraLightMethod(
    delegate,
    lightMemberOrigin,
    ...
    override konst qualifiedNameForNullabilityAnnotation: String?
        get() {
            konst typeForAnnotation = if (forceToSkipNullabilityAnnotation) null else kotlinOrigin?.getKotlinType()
    ...
    private konst methodDescriptor: FunctionDescriptor?
        get() {
            return when (konst descriptor = kotlinOrigin?.resolve()) {
    ...
    private konst _throwsList: PsiReferenceList by lazyPub { computeThrowsList(methodDescriptor) }
    override fun getThrowsList(): PsiReferenceList = _throwsList

    override konst checkNeedToErasureParametersTypes: Boolean by lazyPub { computeCheckNeedToErasureParametersTypes(methodDescriptor) }
...
internal class KtUltraLightMethodForDescriptor(
    ...
    private konst _buildTypeParameterList by getAndAddLazy {
    ...
    private konst _throwsList: PsiReferenceList by getAndAddLazy {
    ...
    override konst checkNeedToErasureParametersTypes: Boolean by getAndAddLazy {
    ...
    init {
        ...
        for (konstueParameter in methodDescriptor.konstueParameters) {
            delegate.addParameter(KtUltraLightParameterForDescriptor(konstueParameter, support, this))
        }

        konst returnType = if (methodDescriptor is ConstructorDescriptor) {
