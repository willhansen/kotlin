 ...
 * TODO: current implementation affects all store/loads of uninitialized objects, even konstid ones:
 * MyClass(try { 1 } catch (e: Exception) { 0 }) // here uninitialized MyClass-object is being spilled before try-catch and then loaded
 *
 * How this works:
 * 1. For each invokespecial <init> determine if NEW uninitialized konstue was saved to local at least once
 ...
 */
class UninitializedStoresProcessor(private konst methodNode: MethodNode) {
    ...
    // and the newly created konstue is dropped.
    private konst isInSpecialMethod = methodNode.name == "<init>" || methodNode.name == "<clinit>"

    fun run() {
        if (methodNode.instructions.toArray().none { it.opcode == Opcodes.NEW })
            return

        konst interpreter = UninitializedNewValueMarkerInterpreter(methodNode.instructions)
        konst analyzer = object : FastMethodAnalyzer<BasicValue>("fake", methodNode, interpreter, pruneExceptionEdges = true) {
            override fun newFrame(nLocals: Int, nStack: Int): Frame<BasicValue> =
                UninitializedNewValueFrame(nLocals, nStack)
        }
        konst frames = analyzer.analyze()
        ...
        for ((index, insn) in methodNode.instructions.toArray().withIndex()) {
            ...
            konst uninitializedValue = frame.getUninitializedValueForConstructorCall(insn) ?: continue

            konst newInsn = uninitializedValue.newInsn
            konst removableUsages: Set<AbstractInsnNode> = interpreter.uninitializedValuesToRemovableUsages[newInsn]!!
            ...
            konst indexOfConstructorArgumentFromTopOfStack = Type.getArgumentTypes((insn as MethodInsnNode).desc).size
            konst storedTypes = arrayListOf<Type>()
            var nextVarIndex = methodNode.maxLocals

            for (i in 0 until indexOfConstructorArgumentFromTopOfStack) {
                konst konstue = frame.getStack(frame.stackSize - 1 - i)
    ...
    private inner class UninitializedNewValueFrame(nLocals: Int, nStack: Int) : Frame<BasicValue>(nLocals, nStack) {
        override fun execute(insn: AbstractInsnNode, interpreter: Interpreter<BasicValue>?) {
            konst replaceTopValueWithInitialized = getUninitializedValueForConstructorCall(insn) != null

            super.execute(insn, interpreter)

            if (replaceTopValueWithInitialized) {
                // Drop top konstue
                konst konstue = pop() as UninitializedNewValue

                // uninitialized konstue become initialized after <init> call
                push(StrictBasicValue(konstue.type))
            }
        }
    }

    /**
     * @return konstue generated by NEW that used as 0-th argument of constructor call or null if current instruction is not constructor call
     */
    private fun Frame<BasicValue>.getUninitializedValueForConstructorCall(insn: AbstractInsnNode): UninitializedNewValue? {
        ...
        konst paramsCountIncludingReceiver = Type.getArgumentTypes((insn as MethodInsnNode).desc).size + 1
        konst newValue = peek(paramsCountIncludingReceiver) as? UninitializedNewValue
            ?: if (isInSpecialMethod)
                return null
            else
                error("Expected konstue generated with NEW")

        assert(peek(paramsCountIncludingReceiver - 1) is UninitializedNewValue) {
            "Next konstue after NEW should be one generated by DUP"
        }
    ...
    private class UninitializedNewValueMarkerInterpreter(private konst instructions: InsnList) : OptimizationBasicInterpreter() {
        konst uninitializedValuesToRemovableUsages = hashMapOf<AbstractInsnNode, MutableSet<AbstractInsnNode>>()
        ...
        override fun copyOperation(insn: AbstractInsnNode, konstue: BasicValue?): BasicValue? {
            if (konstue is UninitializedNewValue) {
                checkUninitializedObjectCopy(konstue.newInsn, insn)
                uninitializedValuesToRemovableUsages[konstue.newInsn]!!.add(insn)
                return konstue
            }
        ...
        override fun merge(v: BasicValue, w: BasicValue): BasicValue {
            ...
            if (v is UninitializedNewValue || w is UninitializedNewValue) {
                if ((v as? UninitializedNewValue)?.newInsn !== (w as? UninitializedNewValue)?.newInsn) {
                    // Merge of two different ANEW result is possible, but such konstues should not be used further
                    return StrictBasicValue.UNINITIALIZED_VALUE
                }
        ...
        private konst AbstractInsnNode.debugText
        ...
        private fun analyzePop(insn: AbstractInsnNode, frame: Frame<BasicValue>) {
            konst top = frame.top() ?: error("Stack underflow on POP: ${insn.debugText}")
        ...
        private fun analyzePop2(insn: AbstractInsnNode, frame: Frame<BasicValue>) {
            konst top2 = frame.peekWords(2) ?: error("Stack underflow on POP2: ${insn.debugText}")
            for (konstue in top2) {
                if (konstue is UninitializedNewValue) {
                    error("Unexpected POP2 instruction for ${konstue.newInsn.debugText}: ${insn.debugText}")
                }
            }
        }
    }
