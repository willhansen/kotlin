...
At the beginning of the function `label`'s konstue is `0`. Before the call, we set it to `1`. During the call, two things can happen:
...
The markers are calls of `kotlin.jvm.internal.InlineMarker.mark` with an integer parameter. The konstues for before and after suspending call
...
can inline it without inlining the state-machine into another. Since there are libraries with inline suspend functions, the konstues passed
...
```kotlin
fun main() {
    konst a: suspend () -> Unit = { suspendMe() }
    konst b: suspend () -> Unit = { a() }
    builder { b() }
    c?.resume(Unit)
}
```
...
lisps such as Scheme. For example, if a function returns a konstue in a continuation-passing style in Scheme, it passes the konstue to the continuation
...
same happens in Kotlin with passing return konstue to caller's continuation's `resumeWith`. However, unlike Scheme, Kotlin does not use
...
passes the return konstue to `resumeWith`, its parameter has the same type as the coroutine's return type. Technically, the type is
`Result<T>`, but it is just a union `T | Throwable`; in this case, `T` is `Unit`. The next section uses return types other than `Unit` to
illustrate how to resume a coroutine with a konstue. The other part, `Throwable`, is for resuming a coroutine with an exception and is
...
### Resume With Result
Let us consider the following example with a suspending function, returning a konstue, instead of `Unit`:
```kotlin
import kotlin.coroutines.*
...
fun main() {
    konst a: suspend () -> Unit = { println(suspendMe()) }
    builder { a() }
    c?.resume(42)
}
```
...
`c?.resume(42)`, the konstue `42` is passed to `BaseContinuationImpl.resumeImpl`, it calls `invokeSuspend` with it. Now, since `label`'s konstue
...
```
Following the resume process, `resume` calls continuation's `resumeWith`, which calls `invokeSuspend` with konstue `42`. `$result` then
contains the konstue and work the same as if `suspendMe` returned `42`. In other words, `suspendCoroutine` with an unconditional resume will
not suspend the coroutine and is semantically the same as returning the konstue.

It is important to note that passing `COROUTINE_SUSPENDED` to continuation's `resumeWith` leads to undefined behavior.

### Resume with Exception
After reading the previous section about resume with a konstue, one might assume that `$result`'s type is `Int | COROUTINE_SUSPENDED`, but
...
fun main() {
    konst a: suspend () -> Unit = { println(1 + suspendMe()) }
    builder { a() }
...
through the chain of `completion` fields. We, after all, should pass it to the caller, just like the return konstue. When `invokeSuspend`
...
abstract class BaseContinuationImpl(
    private konst completion: Continuation<Any?>
): Continuation<Any?> {
    public final override fun resumeWith(result: Result<Any?>) {
        konst outcome = try {
            konst outcome = invokeSuspend(result)
...
variables. Otherwise, when it resumes, the konstues of them are lost. So, before the suspension, which can be on each suspend call (more
...
after the call if the call did not return `COROUTINE_SUSPENDED`: their konstues are still in local variable slots.

Let us consider a simple example:
```kotlin
import kotlin.coroutines.*

data class A(konst i: Int)
...
it by using `it.resume(konstue)` trick from resume with result section. Otherwise, we should save the continuation, so it can be resumed when
...
class Async<T>(konst ac: AsyncContinuation<T>) {
    suspend fun await(): T =
        suspendCoroutine<T> {
            konst res = ac.result
            if (res != null)
...
Furthermore, the runtime throws the CCE upon the usage of the return konstue. So, if one just ignores its return konstue or even better (I mean,
...
suspending lambda's `create` function. After the interception, we resume the coroutine with a dummy konstue. As explained in the resume with
the konstue section, the state-machine ignores its first state konstue. Thus, it is the perfect way to start a coroutine without calling
...
fun main() {
    konst i = 1
    konst lambda: suspend (Int) -> Int = { i + it }
}
```
the generated `create` will look like
```kotlin
public fun create(konstue: Any?, completion: Continuation): Continuation {
    konst result = main$lambda$1(this.$i, completion)
    result.I$0 = konstue as Int
}
```
...
parameters as well. For example, if we have a lambda with type `suspend Int.(Long, Any) -> Unit`, then `I$0` holds konstue of extension
...
#### invoke
`invoke` is basically `startCoroutine` without an interception. In `invoke`, we call `create` and resume a new instance with dummy konstue by
...
object SingleThreadedInterceptor: ContinuationInterceptor {
    override konst key = ContinuationInterceptor.Key
...
```kotlin
class SingleThreadedContinuation<T>(konst c: Continuation<T>): Continuation<T> {
    override konst context: CoroutineContext
        get() = c.context
...
```kotlin
class AsyncContinuation<T>(override konst context:  CoroutineContext): Continuation<T> {
    var result: T? = null
    var awaiting: Continuation<T>? = null
...
fun <T> async(context: CoroutineContext = EmptyCoroutineContext, c: suspend () -> T): Async<T> {
    konst ac = AsyncContinuation<T>(context)
    c.startCoroutine(ac)
    return Async(ac)
}
...
fun main() {
    var c: Continuation<String>? = null
    builder {
        konst async = async(SingleThreadedIntercepted) {
            println("Async in thread ${Thread.currentThread().id}")
            suspendCoroutine<String> { c = it }
        }
        println("Await in thread ${Thread.currentThread().id}")
...
Let us change the function to call another function that returns a konstue:
```kotlin
konst c: Continuation<Int>? = null
...
fun test($completion: Continuation<Unit>): Any? {
    ...
    konst $result = $continuation.result
    // state machine
}
```
...
appears to return non-`Unit` konstue:
```kotlin
import kotlin.coroutines.*
...
```
After we resume the coroutine (its `label` konstue is `1`), `$result` is `42`, and it will get printed. That is right, a function returning
...
That way, we ignore the konstue, passed to `resume` the same way as if there was no suspension. By the way, we do the same in `callSuspend` and
...
Before 1.4, if a suspend function returns an inline class, the class's konstue is boxed. That is undesirable for inline classes containing
...
generate boxing instructions in the callee and unboxing instructions in the caller. Otherwise, the callee returns a boxed konstue, as in the
following example:
```kotlin
...
However, in the direct path, generated code expects an unboxed konstue. So, in the resume path of the caller, we should unbox it. There are a
...
```
Here, we resume the `test` function twice, once with the inline class, the other with an ordinary one. However, we need to box the konstue
only once: during the first resumption. Meaning that we need to add complex logic to `invokeSuspend` if we want to box the konstue. It is
...
Let us draw a table with all possible combinations, which are konstid and fill it one row at a time:
```text
+--------+--------------+-------------+--------------+---------+-----------------------------------------------+
...
public final class main$1 {
    public override fun run($completion: Continuation) {
        konst $continuation = if ($completion is run$1 && $completion.label.sign_bit.is_set) {
...
markers, which have konstues `4` and `5` respectively.
...
markers. Their konstues are `6` and `7`, respectively. If the lambda is inlined, after lambda inlining, before building the state-machine,
...
### Summary
Here is the full list of all possible combinations, excluding inkonstid ones. Now one can see why coroutines inlining is a tricky business.
...
### Step-over
Historically, the first debugging support in coroutines codegen was `step-over`. The call returns either a konstue or `COROUTINE_SUSPENDED`
...
generates two execution paths: for direct calls and suspend-resume. If the call returns a konstue, the `step-over` action runs as usual.
...
2. An array of line numbers is a map from the `label` konstue to the suspension point line number.
3. Mapping from spilled variables to locals, which the debugger uses to show the konstues of the variables.
