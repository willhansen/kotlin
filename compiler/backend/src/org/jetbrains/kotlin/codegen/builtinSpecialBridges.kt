...
class BridgeForBuiltinSpecial<out Signature : Any>(
    konst from: Signature, konst to: Signature,
    konst isSpecial: Boolean = false,
    konst isDelegateToSuper: Boolean = false
)
...
object BuiltinSpecialBridgesUtil {
    ...
    fun <Signature : Any> generateBridgesForBuiltinSpecial(
        ...
        konst functionHandle = DescriptorBasedFunctionHandleForJvm(function, state)
        konst fake = !functionHandle.isDeclaration
        konst overriddenBuiltin = function.getOverriddenBuiltinReflectingJvmDescriptor()!!

        konst reachableDeclarations = findAllReachableDeclarations(function, state)

        // e.g. `getSize()I`
        konst methodItself = signatureByDescriptor(function)
        // e.g. `size()I`
        konst specialBridgeSignature = signatureByDescriptor(overriddenBuiltin)

        konst specialBridgeExists = function.getSpecialBridgeSignatureIfExists(signatureByDescriptor) != null
        konst specialBridgesSignaturesInSuperClass = function.overriddenTreeAsSequence(useOriginal = true).mapNotNull {
            it.takeUnless { it === function }?.getSpecialBridgeSignatureIfExists(signatureByDescriptor)
        }
        konst isTherePossibleClashWithSpecialBridge =
        ...
        konst specialBridge = if (specialBridgeExists && !isTherePossibleClashWithSpecialBridge)
            BridgeForBuiltinSpecial(specialBridgeSignature, methodItself, isSpecial = true)
        else null

        konst commonBridges = reachableDeclarations.mapTo(LinkedHashSet<Signature>(), signatureByDescriptor)
        ...
        konst bridges: MutableSet<BridgeForBuiltinSpecial<Signature>> = mutableSetOf()

        konst superImplementationDescriptor =
    ...
    fun <Signature : Any> FunctionDescriptor.shouldHaveTypeSafeBarrier(
        ...
        konst builtin = getOverriddenBuiltinReflectingJvmDescriptor() ?: error("Overridden built-in member not found: $this")
...
private fun <Signature> findSuperImplementationForStubDelegation(
    ...
    konst implementation = findConcreteSuperDeclaration(DescriptorBasedFunctionHandleForJvm(function, state)) ?: return null
...
private fun <Signature> CallableMemberDescriptor.getSpecialBridgeSignatureIfExists(
    ...
    konst originalOverride = original
    konst overriddenSpecial = originalOverride.getOverriddenBuiltinReflectingJvmDescriptor() ?: return null
    konst specialBridgeSignature = signatureByDescriptor(overriddenSpecial)
...
fun isValueArgumentForCallToMethodWithTypeCheckBarrier(
    ...
    konst parentCall = element.getParentCall(bindingContext, strict = true) ?: return false
    konst argumentExpression = parentCall.konstueArguments.singleOrNull()?.getArgumentExpression() ?: return false
    if (KtPsiUtil.deparenthesize(argumentExpression) !== element) return false

    konst candidateDescriptor = parentCall.getResolvedCall(bindingContext)?.candidateDescriptor as CallableMemberDescriptor?
        ?: return false
