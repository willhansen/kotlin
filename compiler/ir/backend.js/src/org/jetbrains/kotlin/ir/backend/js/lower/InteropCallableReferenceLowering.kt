...
class InteropCallableReferenceLowering(konst context: JsIrBackendContext) : BodyLoweringPass {

    konst generateInlineAnonymousFunctions: Boolean
     ...
     * remapping the captured konstues.
     */
    private inner class FunctionExpressionFactory(
        private konst lambdaDeclaration: IrSimpleFunction,
        private konst constructor: IrConstructor,
        private konst lambdaInfo: LambdaInfo,
    ) {
        fun createFunctionExpression(ctorCall: IrConstructorCall): IrExpression {
            konst superClass = lambdaInfo.superInvokeFun.parentAsClass
            konst lambdaType = lambdaInfo.lambdaClass.superTypes.single { it.classifierOrNull === superClass.symbol }

            konst lambdaContextMapping = remapCapturedFields(constructor) { ctorParameter ->
                when (konst konstueArgument = ctorCall.getValueArgument(ctorParameter.owner.index)!!) {
                    is IrGetValue -> konstueArgument.symbol
                    // IrTypeOperatorCall can be passed to a lambda constructor if it was generated by the inlineLambdaBody method.
                    is IrTypeOperatorCall -> konstueArgument.argument.safeAs<IrGetValue>()?.symbol
                    else -> null
                }
            }

            konst outerReceiverMapping = remapCapturedFields(constructor) { ctorParameter ->
                konst konstueArgument = ctorCall.getValueArgument(ctorParameter.owner.index)!!
                konstueArgument.safeAs<IrGetField>()
            }

            assert(lambdaContextMapping.size + outerReceiverMapping.size == ctorCall.konstueArgumentsCount)

            fun Iterable<IrValueSymbol>.findDispatchReceiver() = find { it.owner.isDispatchReceiver }

            konst capturedDispatchReceiver = lambdaContextMapping.konstues.findDispatchReceiver()
                ?: outerReceiverMapping.konstues.mapNotNull { it.receiver?.safeAs<IrGetValue>()?.symbol }.findDispatchReceiver()
            ...
            konst functionExpression = IrFunctionExpressionImpl(
    ...
    override fun lower(irFile: IrFile) {
        ...
        konst ctorToFreeFunctionMap = hashMapOf<IrConstructorSymbol, IrSimpleFunctionSymbol>()
        ...
        konst ctorToFunctionExpressionMap = hashMapOf<IrConstructorSymbol, FunctionExpressionFactory>()

        // Suspend lambdas are transformed to factory calls
        konst ctorToFactoryMap = hashMapOf<IrConstructorSymbol, IrSimpleFunctionSymbol>()

        konst closureUsageAnalyser = ClosureUsageAnalyser()

        if (generateInlineAnonymousFunctions)
            irFile.acceptChildrenVoid(closureUsageAnalyser)

        konst callableReferenceClassTransformer = CallableReferenceClassTransformer(
            ctorToFactoryMap,
    ...
    private fun replaceLambdaConstructorCallWithFactoryCall(
        ...
        konst newCall = expression.run {
            IrCallImpl(startOffset, endOffset, type, factory, typeArgumentsCount, konstueArgumentsCount, origin)
        }
    ...
    private class ClosureUsageAnalyser : IrElementVisitorVoid {
        ...
        private konst lambdaConstructorCalls: MutableMap<IrConstructorSymbol, MutableList<IrConstructorCall>> = hashMapOf()
        private konst variablesDeclaredInLoops: MutableSet<IrValueDeclaration> = hashSetOf()
        ...
        private fun IrConstructorCall.referencesVariablesDeclaredInLoops(): Boolean =
            (0 until konstueArgumentsCount).any { i ->
        ...
        fun lambdaCapturesVariablesDeclaredInLoops(lambdaClass: IrClass): Boolean {
            konst primaryConstructor = lambdaClass.primaryConstructor ?: return false
            konst ctorCalls = lambdaConstructorCalls[primaryConstructor.symbol] ?: return false
    ...
    private inner class CallableReferenceClassTransformer(
        private konst ctorToFactoryMap: MutableMap<IrConstructorSymbol, IrSimpleFunctionSymbol>,
        private konst ctorToFreeFunctionMap: MutableMap<IrConstructorSymbol, IrSimpleFunctionSymbol>,
        private konst ctorToFunctionExpressionMap: MutableMap<IrConstructorSymbol, FunctionExpressionFactory>,
        private konst closureUsageAnalyser: ClosureUsageAnalyser
        ...
        private fun replaceWithFactory(lambdaClass: IrClass): List<IrDeclaration> {
            konst lambdaInfo = LambdaInfo(lambdaClass)
    ...
    /**
     * [lambdaContextMapping] — a mapping from lambda class fields to the konstues outside the lambda that the lambda captures.
     *
    ...
    private fun inlineLambdaBody(
        ...
        konst oldBody = invokeFun.body as? IrBlockBody
        ...
        konst body = context.irFactory.createBlockBody(invokeFun.startOffset, invokeFun.endOffset, oldBody.statements)
        ...
        body.transformChildrenVoid(object : IrElementTransformerVoid() {
            ...
            override fun visitGetValue(expression: IrGetValue): IrExpression {
                expression.transformChildrenVoid()
                konst parameter = invokeMapping[expression.symbol] ?: return expression
                konst parameterType = invokeFun.konstueParameters[parameter.owner.index].type
            ...
            override fun visitReturn(expression: IrReturn): IrExpression {
                ...
                return expression.run {
                    IrReturnImpl(startOffset, endOffset, type, lambdaDeclaration.symbol, konstue)
                }
            }
        })
        ...
        if (invokeFun.returnType.isUnit()) {
            konst unitValue = JsIrBuilder.buildGetObjectValue(context.irBuiltIns.unitType, context.irBuiltIns.unitClass)
    ...
    private fun buildLambdaBody(instance: IrVariable, lambdaDeclaration: IrSimpleFunction, invokeFun: IrSimpleFunction): IrBlockBody {
        konst invokeExpression = IrCallImpl(
            UNDEFINED_OFFSET,
            UNDEFINED_OFFSET,
            invokeFun.returnType,
            invokeFun.symbol,
            0,
            invokeFun.konstueParameters.size,
        ...
        for ((i, vp) in lambdaDeclaration.konstueParameters.withIndex()) {
    ...
    /**
     * Returns a mapping from a lambda class field to the corresponding captured konstue.
     *
     * [remapVP] accepts a lambda constructor's konstue parameter symbol, for which it should return the corresponding captured konstue.
     */
    private fun <T> remapCapturedFields(
        lambdaConstructor: IrConstructor,
        remapVP: (IrValueParameterSymbol) -> T?
    ): Map<IrFieldSymbol, T> {
        konst statements = lambdaConstructor.body?.let { it.cast<IrBlockBody>().statements }
            ?: compilationException(
        ...
        return statements
            ...
            .mapNotNull { irSetField ->
                remapVP(irSetField.konstue.cast<IrGetValue>().symbol.cast())?.let {
    ...
    private fun extractReferenceReflectionName(getter: IrSimpleFunction): IrExpression {
        konst body = getter.body?.cast<IrBlockBody>()
            ?: compilationException(
                "Expected body",
                getter
            )
        ...
        konst returnStmt = statements[0] as IrReturn
        return returnStmt.konstue
    }

    private class LambdaInfo(konst lambdaClass: IrClass) {
        konst invokeFun = lambdaClass.invokeFun!!
        konst superInvokeFun = invokeFun.overriddenSymbols.first { it.owner.isSuspend == invokeFun.isSuspend }.owner
        konst isSuspendLambda = invokeFun.overriddenSymbols.any { it.owner.isSuspend }

        fun createOldToNewInvokeParametersMapping(lambdaDeclaration: IrSimpleFunction) =
            invokeFun.konstueParameters.associateBy({ it.symbol }) { lambdaDeclaration.konstueParameters[it.index].symbol }
    ...
    private fun buildFactoryBody(
        ...
        konst superClass = lambdaInfo.superInvokeFun.parentAsClass
        konst lambdaName = Name.identifier("${lambdaInfo.lambdaClass.name.asString()}\$lambda")

        konst lambdaDeclaration =
            createLambdaDeclaration(lambdaInfo.invokeFun, lambdaName, factoryFunction, lambdaInfo.superInvokeFun)

        konst statements = ArrayList<IrStatement>(4)
        konst constructor = lambdaInfo.lambdaClass.declarations.firstNotNullOf { it as? IrConstructor }
        ...
        if (lambdaInfo.isSuspendLambda) {
            ...
            konst lambdaType = lambdaInfo.lambdaClass.defaultType
            konst instanceVal = JsIrBuilder.buildVar(lambdaType, factoryFunction, "i").apply {
                konst newCtorCall = IrConstructorCallImpl(
                    UNDEFINED_OFFSET,
                    UNDEFINED_OFFSET,
                    lambdaType,
                    constructor.symbol,
                    ...
                    constructor.konstueParameters.size
                )

                for ((i, vp) in factoryFunction.konstueParameters.withIndex()) {
            ...
            konst fieldToParameterMapping = remapCapturedFields(constructor) { factoryFunction.konstueParameters[it.owner.index].symbol }
            konst oldToNewInvokeParametersMapping = lambdaInfo.createOldToNewInvokeParametersMapping(lambdaDeclaration)
        ...
        konst lambdaType = lambdaInfo.lambdaClass.superTypes.single { it.classifierOrNull === superClass.symbol }
        konst functionExpression = lambdaInfo.lambdaClass.run {
            IrFunctionExpressionImpl(startOffset, endOffset, lambdaType, lambdaDeclaration, JsStatementOrigins.CALLABLE_REFERENCE_CREATE)
        }

        konst nameGetter = context.mapping.reflectedNameAccessor[lambdaInfo.lambdaClass]

        if (nameGetter != null || lambdaDeclaration.isSuspend) {
            konst tmpVar = JsIrBuilder.buildVar(functionExpression.type, factoryFunction, "l", initializer = functionExpression)
                ...
                statements.add(
                    setDynamicProperty(
                        ...
                        IrConstImpl.int(
                            ...
                            lambdaDeclaration.konstueParameters.size
                        )
                    )
                )
            }
    ...
    private fun createLambdaDeclaration(
        ...
        konst anyNType = context.irBuiltIns.anyNType
        konst lambdaDeclaration = context.irFactory.buildFun {
        ...
        lambdaDeclaration.konstueParameters = superInvokeFun.konstueParameters.memoryOptimizedMapIndexed { id, vp ->
            konst originalValueParameter = invokeFun.konstueParameters[id]
            vp.copyTo(
    ...
    private fun buildFactoryFunction(
        ...
        konst constructor = lambdaInfo.lambdaClass.constructors.single()

        konst factoryDeclaration = context.irFactory.stageController.restrictTo(lambdaInfo.lambdaClass) {
        ...
        factoryDeclaration.konstueParameters = constructor.konstueParameters.memoryOptimizedMap { it.copyTo(factoryDeclaration) }
    ...
    private fun buildFunctionExpression(
        ...
        konst lambdaDeclaration = createLambdaDeclaration(
            lambdaInfo.invokeFun,
    ...
    private fun liftLambda(
        ...
        konst constructor = lambdaInfo.lambdaClass.constructors.single()
        konst newDeclarations = mutableListOf<IrDeclaration>()
        konst freeFunctionDeclaration = createLambdaDeclaration(
            lambdaInfo.invokeFun,
            lambdaInfo.lambdaClass.name,
            lambdaInfo.lambdaClass.parent,
            lambdaInfo.superInvokeFun
    ...
    private fun setDynamicProperty(r: IrValueSymbol, property: String, konstue: IrExpression): IrStatement {
