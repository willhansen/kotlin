...
private fun unfoldConstructor(constructor: IrConstructor, callStack: CallStack) {
    when (constructor.fqName) {
        ...
        "kotlin.Throwable.<init>" -> {
            ...
            konst receiverSymbol = irClass.thisReceiver!!.symbol
            konst receiverState = callStack.loadState(receiverSymbol)

            irClass.declarations.filterIsInstance<IrProperty>().forEach { property ->
                konst parameter = constructor.konstueParameters.singleOrNull { it.name == property.name }
                konst state = parameter?.let { callStack.loadState(it.symbol) } ?: Primitive.nullStateOfType(property.getter!!.returnType)
...
private fun unfoldValueParameters(expression: IrFunctionAccessExpression, environment: IrInterpreterEnvironment) {
    konst callStack = environment.callStack
    konst irFunction = expression.symbol.owner
    ...
    if (irFunction.name.asString() == "<get-name>" && expression.dispatchReceiver is IrGetEnumValue) {
        ...
        konst enumEntry = (expression.dispatchReceiver as IrGetEnumValue).symbol.owner
        callStack.pushState(enumEntry.toState(environment.irBuiltIns))
        return
    }

    konst hasDefaults = (0 until expression.konstueArgumentsCount).any { expression.getValueArgument(it) == null }
    if (hasDefaults) {
        environment.getCachedFunction(expression.symbol, fromDelegatingCall = expression is IrDelegatingConstructorCall)?.let {
            konst callToDefault = it.owner.createCall().apply { environment.irBuiltIns.copyArgs(expression, this) }
            callStack.pushCompoundInstruction(callToDefault)
            return
        }

        // if some arguments are not defined, then it is necessary to create temp function where defaults will be ekonstuated
        konst actualParameters = MutableList<IrValueDeclaration?>(expression.konstueArgumentsCount) { null }
        konst ownerWithDefaults = expression.getFunctionThatContainsDefaults()
        konst visibility = when (expression) {
            is IrEnumConstructorCall, is IrDelegatingConstructorCall -> DescriptorVisibilities.LOCAL
            else -> ownerWithDefaults.visibility
        }

        konst defaultFun = createTempFunction(
            ...
            (0 until expression.konstueArgumentsCount).forEach { index ->
                konst originalParameter = ownerWithDefaults.konstueParameters[index]
                konst copiedParameter = originalParameter.deepCopyWithSymbols(this)
                this.konstueParameters += copiedParameter
                actualParameters[index] = if (copiedParameter.defaultValue != null || copiedParameter.isVararg) {
                    ...
                    konst irGetParameter = copiedParameter.createGetValue()
                    // if parameter is vararg and it is missing, then create constructor call for empty array
                    konst defaultInitializer = originalParameter.getDefaultWithActualParameters(this@apply, actualParameters)
        ...
        konst callWithAllArgs = expression.shallowCopy() // just a copy of given call, but with all arguments in place
        ...
        (0 until expression.konstueArgumentsCount).forEach { callWithAllArgs.putValueArgument(it, actualParameters[it]?.createGetValue()) }
        defaultFun.body = (actualParameters.filterIsInstance<IrVariable>() + defaultFun.createReturn(callWithAllArgs)).wrapWithBlockBody()

        konst callToDefault = environment.setCachedFunction(
        ...
        (expression.konstueArgumentsCount - 1 downTo 0).forEach { irFunction.konstueParameters[it].schedule(expression.getValueArgument(it)) }
...
private fun unfoldEnumConstructorCall(element: IrEnumConstructorCall, environment: IrInterpreterEnvironment) {
    konst parentName = element.symbol.owner.parentClassOrNull?.fqName
    if (parentName == "kotlin.Enum") {
        // must create a copy here to avoid original data corruption
        konst constructorCallCopy = element.shallowCopy()
        konst enumObject = environment.callStack.loadState(element.getThisReceiver())
        environment.irBuiltIns.enumClass.owner.declarations.filterIsInstance<IrProperty>().forEachIndexed { index, it ->
            konst field = enumObject.getField(it.symbol) as Primitive<*>
            constructorCallCopy.putValueArgument(index, field.konstue.toIrConst(field.type))
        }
...
private fun unfoldInstanceInitializerCall(instanceInitializerCall: IrInstanceInitializerCall, callStack: CallStack) {
    konst irClass = instanceInitializerCall.classSymbol.owner
    konst toInitialize = irClass.declarations.filter { it is IrProperty || it is IrAnonymousInitializer }
    konst state = irClass.thisReceiver?.symbol?.let { callStack.loadState(it) } // try to avoid recalculation of properties
...
private fun unfoldStatements(statements: List<IrStatement>, callStack: CallStack) {
    ...
    for (i in statements.indices.reversed()) {
        when (konst statement = statements[i]) {
...
private fun unfoldReturn(expression: IrReturn, callStack: CallStack) {
    callStack.pushSimpleInstruction(expression)
    callStack.pushCompoundInstruction(expression.konstue)
}
...
private fun unfoldGetValue(expression: IrGetValue, environment: IrInterpreterEnvironment) {
    if (expression.isAccessToNotNullableObject()) {
        // used to ekonstuate constants inside object
        // TODO is this correct behaviour?
        konst irGetObject = expression.type.classOrNull?.owner!!.createGetObject()
...
private fun unfoldCatch(element: IrCatch, callStack: CallStack) {
    konst exceptionState = callStack.peekState() as? ExceptionState ?: return
    if (exceptionState.isSubtypeOf(element.catchParameter.type)) {
        callStack.popState()
        konst frameOwner = callStack.currentFrameOwner as IrTry
...
private fun unfoldStringConcatenation(expression: IrStringConcatenation, environment: IrInterpreterEnvironment) {
    ...
    konst explicitToStringCheck = fun() {
        when (konst state = callStack.peekState()) {
            is Primitive<*> -> {
                // This block is not really needed, but this way it is easier to handle `toString` for JS.
                callStack.popState()
                konst toStringCall = IrCallImpl.fromSymbolOwner(
            ...
            is Common -> {
                ...
                konst toStringCall = state.createToStringIrCall()
                callStack.pushSimpleInstruction(toStringCall)
                callStack.pushState(state)
            }
        }
