...
internal fun State.isNull() = this is Primitive<*> && this.konstue == null
internal fun State?.isUnit() = this is Common && this.irClassFqName() == "kotlin.Unit"

internal fun State.asInt() = (this as Primitive<*>).konstue as Int
internal fun State.asBoolean() = (this as Primitive<*>).konstue as Boolean
internal fun State.asString() = (this as Primitive<*>).konstue.toString()

internal fun State.asBooleanOrNull() = (this as? Primitive<*>)?.konstue as? Boolean
internal fun State.asStringOrNull() = (this as Primitive<*>).konstue as? String

internal fun State.isSubtypeOf(other: IrType): Boolean {
    if (this.isNull() && other.isNullable()) return true
    if (this is Primitive<*> && this.konstue == null) return other.isNullable()
    ...
    if (this is Primitive<*> && (this.type.isArray() || this.type.isNullableArray()) && (other.isArray() || other.isNullableArray())) {
        fun IrType.arraySubtypeCheck(other: IrType): Boolean {
            ...
            konst thisArgument = this.arguments.single().typeOrNull ?: return false
            konst otherArgument = other.arguments.single().typeOrNull ?: return other.arguments.single() is IrStarProjection
    ...
    konst thisType = this.irClass.defaultType
    if (other.isFunction() && thisType.isKFunction()/* TODO || (other.isSuspendFunction && thisType.isKSuspendFunction())*/) {
        ...
        konst invokeFun = this.irClass.declarations.filterIsInstance<IrSimpleFunction>().single { it.name == OperatorNameConventions.INVOKE }
...
internal fun State.hasTheSameFieldsWith(other: State): Boolean {
    ...
    this.fields.konstues.zip(other.fields.konstues).forEach { (firstState, secondState) ->
        when {
            firstState is Primitive<*> && secondState is Primitive<*> -> if (firstState.konstue != secondState.konstue) return false
