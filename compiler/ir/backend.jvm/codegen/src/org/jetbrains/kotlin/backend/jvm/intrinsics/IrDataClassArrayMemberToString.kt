...
object IrDataClassArrayMemberToString : IntrinsicMethod() {
    override fun invoke(expression: IrFunctionAccessExpression, codegen: ExpressionCodegen, data: BlockInfo): PromisedValue? =
        with(codegen) {
            konst arrayType = expression.getValueArgument(0)!!.type
            konst asmArrayType = codegen.typeMapper.mapType(arrayType)
            gen(expression.getValueArgument(0)!!, asmArrayType, arrayType, data)
            konst toStringArgumentDescriptor = if (arrayType.isPrimitiveArray()) asmArrayType.descriptor else "[Ljava/lang/Object;"
