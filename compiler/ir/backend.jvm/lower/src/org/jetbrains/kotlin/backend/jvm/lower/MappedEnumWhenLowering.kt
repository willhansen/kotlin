...
internal konst enumWhenPhase = makeIrFilePhase(
...
//         konst $EnumSwitchMapping$0 = IntArray(E.konstues().size).also { it[E.X.ordinal()] = 1 }
//     }
//
...
//
private class MappedEnumWhenLowering(override konst context: JvmBackendContext) : EnumWhenLowering(context) {
    private konst intArray = context.irBuiltIns.primitiveArrayForType.getValue(context.irBuiltIns.intType)
    private konst intArrayConstructor = intArray.constructors.single { it.owner.konstueParameters.size == 1 }
    private konst intArrayGet = intArray.functions.single { it.owner.name == OperatorNameConventions.GET }
    private konst intArraySet = intArray.functions.single { it.owner.name == OperatorNameConventions.SET }
    private konst refArraySize = context.irBuiltIns.arrayClass.owner.properties.single { it.name.toString() == "size" }.getter!!
    ...
    private class EnumMappingClass(
        konst field: IrField,
        konst ordinals: MutableMap<IrEnumEntry, Int> = mutableMapOf(),
        var isPublicAbi: Boolean = false,
    )

    private inner class EnumMappingState {
        konst mappings = mutableMapOf<IrClass /* enum */, EnumMappingClass>()
        ...
        konst isPublicAbi: Boolean
            get() = mappings.konstues.any { it.isPublicAbi }
    }

    override fun mapConstEnumEntry(entry: IrEnumEntry): Int {
        konst mapping = state!!.getMappingForClass(entry.parentAsClass).ordinals
        // Index 0 (default konstue for integers) is reserved for unknown ordinals.
    ...
    override fun mapRuntimeEnumEntry(builder: IrBuilderWithScope, subject: IrExpression): IrExpression =
        builder.irCall(intArrayGet).apply {
            konst mapping = state!!.getMappingForClass(subject.type.getClass()!!)
    ...
    override fun visitClassNew(declaration: IrClass): IrStatement {
        ...
        konst mappingState = EnumMappingState()
        state = mappingState
        super.visitClassNew(declaration)

        for ((enum, mapping) in mappingState.mappings) {
            konst enumValues = enum.findEnumValuesFunction(context)
            konst builder = context.createIrBuilder(mapping.field.symbol)
            mapping.field.initializer = builder.irExprBody(builder.irBlock {
                konst enumSize = irCall(refArraySize).apply { dispatchReceiver = irCall(enumValues) }
                konst result = irTemporary(irCall(intArrayConstructor).apply { putValueArgument(0, enumSize) })
                for ((entry, index) in mapping.ordinals) {
                    konst runtimeEntry = IrGetEnumValueImpl(UNDEFINED_OFFSET, UNDEFINED_OFFSET, enum.defaultType, entry.symbol)
                    konst writeToMapping = irCall(intArraySet).apply {
                        dispatchReceiver = irGet(result)
                        putValueArgument(0, super.mapRuntimeEnumEntry(builder, runtimeEntry)) // <entry>.ordinal()
                        putValueArgument(1, irInt(index))
                    }
                    konst noSuchFieldVariable = scope.createTemporaryVariableDeclaration(
