...
internal konst toArrayPhase = makeIrFilePhase(
    ::ToArrayLowering,
    name = "ToArray",
    description = "Handle toArray functions"
)

private class ToArrayLowering(private konst context: JvmBackendContext) : ClassLoweringPass {
    private konst symbols = context.ir.symbols

    override fun lower(irClass: IrClass) {
        if (irClass.isJvmInterface || !irClass.isCollectionSubClass) return

        konst indirectCollectionSubClass = generateSequence(irClass.superClass, IrClass::superClass).firstOrNull {
        ...
        irClass.findOrCreate(indirectCollectionSubClass, { it.isGenericToArray(context) }) { bridge ->
            irClass.addFunction {
                ...
                konst elementType = addTypeParameter {
                    name = Name.identifier("T")
                    origin = JvmLoweredDeclarationOrigin.TO_ARRAY
                    superTypes.add(context.irBuiltIns.anyNType)
                }
                returnType = context.irBuiltIns.arrayClass.typeWith(elementType.defaultType)
                konst receiver = addDispatchReceiver {
                    type = irClass.defaultType
                    origin = JvmLoweredDeclarationOrigin.TO_ARRAY
                }
                konst prototype = addValueParameter("array", returnType, JvmLoweredDeclarationOrigin.TO_ARRAY)
    ...
    private fun IrClass.findOrCreate(
        ...
        konst existing = functions.find(matcher)
        if (existing != null) {
...
internal konst IrClass.isCollectionSubClass: Boolean
...
private fun IrType.isArrayOrNullableArrayOf(context: JvmBackendContext, element: IrClassifierSymbol): Boolean =
    this is IrSimpleType && (isArray() || isNullableArray()) && arguments.size == 1 && element == when (konst it = arguments[0]) {
...
internal fun IrSimpleFunction.isGenericToArray(context: JvmBackendContext): Boolean =
    name.asString() == "toArray" && typeParameters.size == 1 && konstueParameters.size == 1 &&
            extensionReceiverParameter == null &&
            returnType.isArrayOrNullableArrayOf(context, typeParameters[0].symbol) &&
            konstueParameters[0].type.isArrayOrNullableArrayOf(context, typeParameters[0].symbol)
...
internal fun IrSimpleFunction.isNonGenericToArray(): Boolean =
    name.asString() == "toArray" && typeParameters.isEmpty() && konstueParameters.isEmpty() &&
