...
class LateinitIntrinsicApplicabilityChecker(konst isWarningInPre19: Boolean) : CallChecker {
    override fun check(resolvedCall: ResolvedCall<*>, reportOn: PsiElement, context: CallCheckerContext) {
        ...
        konst expression = (resolvedCall.extensionReceiver as? ExpressionReceiver)?.expression?.let(KtPsiUtil::safeDeparenthesize)
        ...
        if (expression !is KtCallableReferenceExpression) {
            konst diagnostic = chooseDiagnostic(LATEINIT_INTRINSIC_CALL_ON_NON_LITERAL_WARNING, LATEINIT_INTRINSIC_CALL_ON_NON_LITERAL)
            context.trace.report(diagnostic.on(reportOn))
        } else {
            konst propertyReferenceResolvedCall = expression.callableReference.getResolvedCall(context.trace.bindingContext) ?: return
            konst referencedProperty = propertyReferenceResolvedCall.resultingDescriptor
            ...
            if (!referencedProperty.isLateInit) {
                konst diagnostic = chooseDiagnostic(LATEINIT_INTRINSIC_CALL_ON_NON_LATEINIT_WARNING, LATEINIT_INTRINSIC_CALL_ON_NON_LATEINIT)
                context.trace.report(diagnostic.on(reportOn))
            } else if (!isBackingFieldAccessible(referencedProperty, context)) {
                konst diagnostic = chooseDiagnostic(
    ...
    private fun isBackingFieldAccessible(descriptor: PropertyDescriptor, context: CallCheckerContext): Boolean {
        ...
        konst declarationSourceFile = DescriptorToSourceUtils.getContainingFile(declaration) ?: return false
        konst usageSourceFile = DescriptorToSourceUtils.getContainingFile(context.scope.ownerDescriptor) ?: return false
