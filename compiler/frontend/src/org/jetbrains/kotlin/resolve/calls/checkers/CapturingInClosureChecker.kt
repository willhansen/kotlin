...
class CapturingInClosureChecker : CallChecker {
    override fun check(resolvedCall: ResolvedCall<*>, reportOn: PsiElement, context: CallCheckerContext) {
        konst variableResolvedCall = (resolvedCall as? VariableAsFunctionResolvedCall)?.variableCall ?: resolvedCall
        konst variableDescriptor = variableResolvedCall.resultingDescriptor as? VariableDescriptor
    ...
    private fun checkCapturingInClosure(variable: VariableDescriptor, trace: BindingTrace, scope: LexicalScope) {
        konst variableParent = variable.containingDeclaration
        konst scopeContainer = scope.ownerDescriptor
    ...
    private fun checkFieldInExactlyOnceLambdaInitialization(
        ...
        konst scopeDeclaration = DescriptorToSourceUtils.descriptorToDeclaration(scopeContainer) as? KtFunction ?: return
        ...
        konst (callee, param) = getCalleeDescriptorAndParameter(trace.bindingContext, scopeDeclaration) ?: return
    ...
    private fun isLhsOfAssignment(nameElement: KtExpression): Boolean {
        konst parent = nameElement.parent as? KtBinaryExpression ?: return false
    ...
    private fun isCapturedVariable(variableParent: DeclarationDescriptor, scopeContainer: DeclarationDescriptor): Boolean {
        ...
        if (variableParent is ConstructorDescriptor) {
            konst classDescriptor = variableParent.containingDeclaration
    ...
    private fun getCaptureKind(
        ...
        konst scopeDeclaration = DescriptorToSourceUtils.descriptorToDeclaration(scopeContainer)
        ...
        if (InlineUtil.isInlinedArgument(scopeDeclaration as KtFunction, context, false) &&
            ...
            konst scopeContainerParent = scopeContainer.containingDeclaration ?: error("parent is null for $scopeContainer")
            return if (
        ...
        konst exactlyOnceContract = isExactlyOnceContract(context, scopeDeclaration)
        ...
        // exceptions inside catch blocks ans konsts in when
    ...
    private fun isArgument(variable: VariableDescriptor, variableParent: DeclarationDescriptor): Boolean =
        variable is ValueParameterDescriptor && variableParent is CallableDescriptor
                && variableParent.konstueParameters.contains(variable)

    private fun isValInWhen(variable: VariableDescriptor): Boolean {
        konst psi = ((variable as? LocalVariableDescriptor)?.source as? KotlinSourceElement)?.psi ?: return false
    ...
    private fun isForLoopParameter(variable: VariableDescriptor): Boolean {
        ...
        if (psi.parent is KtForExpression) {
            konst forExpression = psi.parent as KtForExpression
            return forExpression.loopParameter == psi
        } else if (psi.parent is KtDestructuringDeclaration) {
            konst parameter = psi.parent.parent as? KtParameter ?: return false
            konst forExpression = parameter.parent as? KtForExpression ?: return false
    ...
    private fun isExactlyOnceParameter(function: DeclarationDescriptor, parameter: VariableDescriptor): Boolean {
        ...
        konst contractDescription = function.getUserData(ContractProviderKey)?.getContractDescription() ?: return false
        konst effect = contractDescription.effects.filterIsInstance<CallsEffectDeclaration>()
    ...
    private fun isExactlyOnceContract(bindingContext: BindingContext, argument: KtFunction): Boolean {
        konst (descriptor, parameter) = getCalleeDescriptorAndParameter(bindingContext, argument) ?: return false
    ...
    private fun getCalleeDescriptorAndParameter(
        ...
        konst call = KtPsiUtil.getParentCallIfPresent(argument) ?: return null
        konst resolvedCall = call.getResolvedCall(bindingContext) ?: return null
        konst descriptor = resolvedCall.resultingDescriptor
        konst konstueArgument = resolvedCall.call.getValueArgumentForExpression(argument) ?: return null
        konst mapping = resolvedCall.getArgumentMapping(konstueArgument) as? ArgumentMatch ?: return null
        konst parameter = mapping.konstueParameter
...
fun findDestructuredVariable(variable: VariableDescriptor, variableParent: DeclarationDescriptor): ValueParameterDescriptor? =
    if (variable is LocalVariableDescriptor && variableParent is AnonymousFunctionDescriptor) {
        variableParent.konstueParameters.find {
