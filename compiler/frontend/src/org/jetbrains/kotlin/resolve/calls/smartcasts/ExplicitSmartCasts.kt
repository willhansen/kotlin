...
interface ExplicitSmartCasts {
    ...
    konst defaultType: KotlinType?

    operator fun plus(smartCast: SingleSmartCast): ExplicitSmartCasts
}

data class SingleSmartCast(konst call: Call?, konst type: KotlinType) : ExplicitSmartCasts {
    override fun type(call: Call?) = if (call == this.call) type else null

    override konst defaultType: KotlinType get() = type
...
data class MultipleSmartCasts internal constructor(konst map: Map<Call?, KotlinType>) : ExplicitSmartCasts {
    override fun type(call: Call?) = map[call]

    override konst defaultType: KotlinType? get() = null
