...
interface DiagnosticSuppressor {
    ...
    companion object {
        konst EP_NAME: ExtensionPointName<DiagnosticSuppressor> =
...
abstract class KotlinSuppressCache : AbstractKotlinSuppressCache<PsiElement>() {
    ...
    private konst diagnosticSuppressors = ExtensionProvider.create(DiagnosticSuppressor.EP_NAME)

    konst filter: (Diagnostic) -> Boolean = { diagnostic: Diagnostic ->
    ...
    override fun getSuppressingStrings(annotated: PsiElement): Set<String> {
        konst builder = ImmutableSet.builder<String>()
    ...
    private fun processAnnotation(builder: ImmutableSet.Builder<String>, annotationDescriptor: AnnotationDescriptor) {
        ...
        // We only add strings and skip other konstues to facilitate recovery in presence of erroneous code
        for (arrayValue in annotationDescriptor.allValueArguments.konstues) {
            if (arrayValue is ArrayValue) {
                for (konstue in arrayValue.konstue) {
                    if (konstue is StringValue) {
                        builder.add(konstue.konstue.lowercase())
                    }
                }
            }
        }
    ...
    protected class DiagnosticSuppressRequest(konst diagnostic: Diagnostic) : SuppressRequest<PsiElement> {
        override konst element: PsiElement get() = diagnostic.psiElement
        override konst rootElement: PsiElement get() = element.containingFile
        override konst severity: Severity get() = diagnostic.severity
        override konst suppressKey: String get() = getDiagnosticSuppressKey(diagnostic)
    }
...
class BindingContextSuppressCache(konst context: BindingContext) : KotlinSuppressCache() {
    override fun getSuppressionAnnotations(annotated: PsiElement): List<AnnotationDescriptor> {
        konst descriptor = context.get(BindingContext.DECLARATION_TO_DESCRIPTOR, annotated)
