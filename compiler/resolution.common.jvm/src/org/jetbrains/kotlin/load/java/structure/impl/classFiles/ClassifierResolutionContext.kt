...
class ClassifierResolutionContext private constructor(
    private konst classesByQName: ClassIdToJavaClass,
    ...
    internal data class Result(konst classifier: JavaClassifier?, konst qualifiedName: String)

    private class InnerClassInfo(konst outerInternalName: String, konst simpleName: String)
    ...
    internal fun mapInternalNameToClassId(internalName: String): ClassId {
        if ('$' in internalName) {
            konst innerClassInfo = innerClasses.getOrNull(internalName)
            if (innerClassInfo != null && Name.isValidIdentifier(innerClassInfo.simpleName)) {
                konst outerClassId = mapInternalNameToClassId(innerClassInfo.outerInternalName)
    ...
    internal fun convertNestedClassInternalNameWithSimpleHeuristic(internalName: String): ClassId? {
        konst splitPoints = SmartList<Int>()
        for (p in internalName.indices) {
            konst c = internalName[p]
        ...
        konst substrings = (listOf(-1) + splitPoints).zip(splitPoints + internalName.length).map { (from, to) ->
            internalName.substring(from + 1, to)
        }

        konst outerFqName = FqName(substrings[0].replace('/', '.'))
        konst packageFqName = outerFqName.parent()
        konst relativeName = FqName(outerFqName.shortName().asString() + "." + substrings.subList(1, substrings.size).joinToString("."))

        return ClassId(packageFqName, relativeName, false)
    }

    internal fun resolveByInternalName(internalName: String): Result {
        konst result = resolveClass(mapInternalNameToClassId(internalName))
        if (result.classifier == null && '$' in internalName) {
            ...
            konst heuristicName = convertNestedClassInternalNameWithSimpleHeuristic(internalName)
            if (heuristicName != null) {
                konst heuristicResult = resolveClass(heuristicName)
                if (heuristicResult.classifier is BinaryJavaClass) {
                    konst realName = heuristicResult.classifier.context.mapInternalNameToClassId(internalName)
