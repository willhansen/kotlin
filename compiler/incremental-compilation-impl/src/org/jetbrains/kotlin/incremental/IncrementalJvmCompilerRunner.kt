...
open class IncrementalJvmCompilerRunner(
    ...
    private konst usePreciseJavaTracking: Boolean,
    buildHistoryFile: File,
    outputDirs: Collection<File>?,
    private konst modulesApiHistory: ModulesApiHistory,
    override konst kotlinSourceFilesExtensions: List<String> = DEFAULT_KOTLIN_SOURCE_FILES_EXTENSIONS,
    private konst classpathChanges: ClasspathChanges,
    ...
    private konst messageCollector = BufferingMessageCollector()
    private konst compilerConfiguration: CompilerConfiguration by lazy {
        konst filterMessageCollector = FilteringMessageCollector(messageCollector) { !it.isError }
    ...
    private konst psiFileProvider = object {
        ...
        private konst psiFileFactory: PsiFileFactory by lazy {
    ...
    private konst changedUntrackedJavaClasses = mutableSetOf<ClassId>()
    ...
    override fun setupJarDependencies(args: K2JVMCompilerArguments, reporter: BuildReporter): Map<String, AbiSnapshot> {
        //fill abiSnapshots
        konst abiSnapshots = HashMap<String, AbiSnapshot>()
    ...
    private fun calculateSourcesToCompileImpl(
        ...
        konst changedAndImpactedSymbols = when (classpathChanges) {
            ...
            is ToBeComputedByIncrementalCompiler -> reporter.measure(BuildTime.COMPUTE_CLASSPATH_CHANGES) {
                ...
                konst storeCurrentClasspathSnapshotForReuse =
                ...
                konst classpathChanges = computeClasspathChanges(
            ...
            is ClasspathSnapshotDisabled -> reporter.measure(BuildTime.IC_ANALYZE_CHANGES_IN_DEPENDENCIES) {
                ...
                konst lastBuildInfo = BuildInfo.read(lastBuildInfoFile, messageCollector)
                    ?: return CompilationMode.Rebuild(BuildAttribute.INVALID_LAST_BUILD_INFO)
                reporter.debug { "Last Kotlin Build info -- $lastBuildInfo" }
                konst scopes = caches.lookupCache.lookupSymbols.map { it.scope.ifBlank { it.name } }.distinct()
        ...
        reporter.measure(BuildTime.IC_ANALYZE_CHANGES_IN_JAVA_SOURCES) {
            if (!usePreciseJavaTracking) {
                konst javaFilesChanges = javaFilesProcessor!!.process(changedFiles)
                konst affectedJavaSymbols = when (javaFilesChanges) {
                ...
                konst rebuildReason = processChangedJava(changedFiles, caches)
                if (rebuildReason != null) return CompilationMode.Rebuild(rebuildReason)
            }
        }

        konst androidLayoutChanges = reporter.measure(BuildTime.IC_ANALYZE_CHANGES_IN_ANDROID_LAYOUTS) {
            processLookupSymbolsForAndroidLayouts(changedFiles)
        }
        konst removedClassesChanges = reporter.measure(BuildTime.IC_DETECT_REMOVED_CLASSES) {
    ...
    private fun ProgramSymbolSet.toChangeInfoList(): List<ChangeInfo> {
        konst changes = mutableListOf<ChangeInfo>()
    ...
    private fun processChangedJava(changedFiles: ChangedFiles.Known, caches: IncrementalJvmCachesManager): BuildAttribute? {
        konst javaFiles = (changedFiles.modified + changedFiles.removed).filter(File::isJavaFile)
        ...
        for (javaFile in javaFiles) {
            if (!caches.platformCache.isTrackedFile(javaFile)) {
                ...
                konst psiFile = psiFileProvider.javaFile(javaFile)
                ...
                for (psiClass in psiFile.classes) {
                    konst qualifiedName = psiClass.qualifiedName
    ...
    private fun processChangedUntrackedJavaClass(psiClass: PsiClass, classId: ClassId) {
        ...
        for (innerClass in psiClass.innerClasses) {
            konst name = innerClass.name ?: continue
    ...
    override fun additionalDirtyFiles(
        ...
        konst cache = caches.platformCache
        konst result = HashSet<File>()

        fun partsByFacadeName(facadeInternalName: String): List<File> {
            konst parts = cache.getStableMultifileFacadeParts(facadeInternalName) ?: emptyList()
        ...
        for (generatedFile in generatedFiles) {
            ...
            konst outputClass = generatedFile.outputClass

            when (outputClass.classHeader.kind) {
                KotlinClassHeader.Kind.CLASS -> {
                    konst fqName = outputClass.className.fqNameForClassNameWithoutDollars
                    konst cachedSourceFile = cache.getSourceFileIfClass(fqName)
    ...
    override fun makeServices(
        ...
        super.makeServices(args, lookupTracker, expectActualTracker, caches, dirtySources, isIncremental).apply {
            konst moduleName = requireNotNull(args.moduleName) { "'moduleName' is null!" }
            konst targetId = TargetId(moduleName, "java-production")
            konst targetToCache = mapOf(targetId to caches.platformCache)
            konst incrementalComponents = IncrementalCompilationComponentsImpl(targetToCache)
            register(IncrementalCompilationComponents::class.java, incrementalComponents)
            if (usePreciseJavaTracking) {
                konst changesTracker = JavaClassesTrackerImpl(
    ...
    override fun runCompiler(
        ...
        konst freeArgsBackup = args.freeArgs.toList()
        args.freeArgs += sourcesToCompile.map { it.absolutePath }
        args.allowNoSourceFiles = true
        konst exitCode = compiler.exec(messageCollector, services, args)
