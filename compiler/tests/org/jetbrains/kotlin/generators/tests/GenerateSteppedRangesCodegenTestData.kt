...
object GenerateSteppedRangesCodegenTestData {
    private konst TEST_DATA_DIR = File("compiler/testData/codegen/box/ranges/stepped")
    private konst UNSIGNED_TEST_DATA_DIR = File(TEST_DATA_DIR, "unsigned")

    private konst PREAMBLE_MESSAGE = "Auto-generated by ${GenerateSteppedRangesCodegenTestData::class.java.simpleName}. Do not edit!"
    private konst KT_34166_HEADER = """
    ...
    private konst KT_34166_AFFECTED_FILENAMES = setOf("illegalStepZero.kt", "illegalStepNegative.kt", "illegalStepNonConst.kt")

    private konst JVM_IR_FAILING_FOR_UNSIGNED_FILENAMES = setOf<String>()
    private konst USE_OLD_MANGLING_SCHEME = emptySet<String>()

    private enum class Type(konst type: String, konst isLong: Boolean = false, konst isUnsigned: Boolean = false) {
        INT("Int"),
        ...
        fun convertValue(i: Any, forStep: Boolean = false) = when (i) {
            ...
            // For convenience, replace the type in konstue expressions so we can re-use the same builder for different types
            else -> i.toString().replace("U?Int\\.".toRegex(), "$type.")
        }
    }

    enum class Function(konst infixFunctionName: String, konst subdir: String) {
        RANGE_TO("..", "rangeTo"),
    ...
    private class TestBuilder(
        ...
        konst operations: MutableList<Operation> = mutableListOf(),
        expectedValuesOrFailIfNull: List<Any>? = null
    ) {
        konst first = firstLast.first
        konst last = firstLast.second
        var expectedValuesOrFailIfNull: List<Any>? = expectedValuesOrFailIfNull
            private set

        private abstract class Operation
        private class Step(konst step: Int) : Operation()
        private class StepExpression(konst expression: String) : Operation()
    ...
    private fun generateTests(
        ...
        // For convenience, derive test for "downTo" by swapping "first"/"last" and reversing expected konstues
    ...
    private fun generateTestsForRangeToAndUntil(
        ...
        konst last = rangeToBuilder.last as Int
    ...
    private fun generateTestsForFunction(
        ...
        extraCode: String? = null,
        subdir: String? = null
    ) = generateTestsForFunction(fileName, Type.konstues().associate { it to builder }, function, extraCode, subdir)
    ...
    private fun generateTestsForFunction(
        ...
        konst fullSubdirPath = buildString {
        ...
        konst (unsignedTests, signedTests) = typeToBuilderMap.asSequence().partition { (type, _) -> type.isUnsigned }
    ...
    private fun PrintWriter.printTestForFunctionAndType(builder: TestBuilder, function: Function, type: Type, asLiteral: Boolean) {
        konst shouldFail = (builder.expectedValuesOrFailIfNull == null)
        konst listVarName = type.type.lowercase() + "List"
        if (shouldFail) {
            println("    assertFailsWith<IllegalArgumentException> {")
        } else {
            println("    konst $listVarName = mutableListOf<${type.type}>()")
        }

        konst indent = if (shouldFail) "    " else ""
        if (asLiteral) {
            println("$indent    for (i in ${builder.buildFullLiteral(type, function)}) {")
        } else {
            konst progressionVarName = type.type.lowercase() + "Progression"
            println("$indent    konst $progressionVarName = ${builder.buildRangeOnlyExpression(type, function)}")
