...
class FirSessionConfigurator(private konst session: FirSession) {
    private konst registeredExtensions: MutableList<BunchOfRegisteredExtensions> = mutableListOf(BunchOfRegisteredExtensions.empty())
    ...
    fun configure() {
        ...
        if (session.kind == FirSession.Kind.Library) {
            konst filteredExtensions = extensions.extensions.filterKeys { it in FirExtensionRegistrar.ALLOWED_EXTENSIONS_FOR_LIBRARY_SESSION }
