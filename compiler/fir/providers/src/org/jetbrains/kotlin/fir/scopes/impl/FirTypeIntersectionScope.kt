...
class FirTypeIntersectionScope private constructor(
    ...
    private konst scopes: List<FirTypeScope>,
    dispatchReceiverType: ConeSimpleKotlinType,
) : AbstractFirOverrideScope(session, overrideChecker) {
    private konst intersectionContext =
        FirTypeIntersectionScopeContext(session, overrideChecker, scopes, dispatchReceiverType, forClassUseSiteScope = false)

    private konst overriddenSymbols: MutableMap<FirCallableSymbol<*>, Collection<MemberWithBaseScope<FirCallableSymbol<*>>>> = mutableMapOf()
    ...
    private konst classifiersNamesCached by lazy(LazyThreadSafetyMode.PUBLICATION) {
    ...
    private inline fun <D : FirCallableSymbol<*>> processCallablesByName(
        ...
        konst callablesWithOverridden = intersectionContext.collectIntersectionResultsForCallables(name, processCallables)

        for (resultOfIntersection in callablesWithOverridden) {
            konst symbol = resultOfIntersection.chosenSymbol
    ...
    fun <S : FirCallableSymbol<*>> getDirectOverriddenSymbols(symbol: S): Collection<MemberWithBaseScope<S>> {
        konst intersectionOverride = intersectionContext.intersectionOverrides.getValueIfComputed(symbol)
        konst allDirectOverridden = overriddenSymbols[symbol].orEmpty() + intersectionOverride?.let {
