...
abstract class FirAssignExpressionAltererExtension(session: FirSession) : FirExtension(session) {
    companion object {
        konst NAME = FirExtensionPointName("AssignAlterer")
    }

    final override konst name: FirExtensionPointName
        get() = NAME

    final override konst extensionType: KClass<out FirExtension> = FirAssignExpressionAltererExtension::class

    /**
     * At this point [variableAssignment] contains resolved and completed lhs and calleeReference(lkonstue)
...
konst FirExtensionService.assignAltererExtensions: List<FirAssignExpressionAltererExtension> by FirExtensionService.registeredExtensions()
