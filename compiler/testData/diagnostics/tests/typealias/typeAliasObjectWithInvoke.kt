// FIR_IDENTICAL
// NI_EXPECTED_FILE

object ObjectWithInvoke {
    operator fun invoke() = this
}

class ClassWithCompanionObjectWithInvoke {
    companion object {
        operator fun invoke(x: Any) = x
    }
}

typealias WI = ObjectWithInvoke

typealias CWI = ClassWithCompanionObjectWithInvoke

konst test1 = WI()
konst test2 = WI(<!TOO_MANY_ARGUMENTS!>null<!>)

konst test3 = CWI()
konst test4 = CWI("")
konst test5 = CWI(<!NULL_FOR_NONNULL_TYPE!>null<!>)
konst test5a = ClassWithCompanionObjectWithInvoke(<!NULL_FOR_NONNULL_TYPE!>null<!>)
