...
fun main() {
    konst array = MyArray()
    konst f: () -> String = <!INITIALIZER_TYPE_MISMATCH!>{
        <!TYPE_MISMATCH!>array[2] = 23<!> //error: Type mismatch: inferred type is Int (!!!) but String was expected
    }<!>
    konst g: () -> String = <!INITIALIZER_TYPE_MISMATCH!>{
        var x = 1
        x += 2  //no error, but it should be here
    }<!>
    konst h: () -> String = <!INITIALIZER_TYPE_MISMATCH!>{
        var x = 1
        x = 2  //the same
    }<!>
    konst array1 = MyArray1()
    konst i: () -> String = <!INITIALIZER_TYPE_MISMATCH!>{
        <!TYPE_MISMATCH!>array1[2] = 23<!>
    }<!>

    konst fi: () -> String = <!INITIALIZER_TYPE_MISMATCH!>{
        <!TYPE_MISMATCH!>array[2] = 23<!>
    }<!>
    konst gi: () -> String = <!INITIALIZER_TYPE_MISMATCH!>{
        var x = 1
        x += 21
    }<!>
