...
fun test_1() {
    konst x1: String.(String) -> String = <!INITIALIZER_TYPE_MISMATCH!>{ str, <!CANNOT_INFER_PARAMETER_TYPE!>str2<!> -> "this" }<!>
    konst x2: String.() -> String = { <!UNRESOLVED_REFERENCE!>it<!> }
    konst x3: String.() -> String = <!INITIALIZER_TYPE_MISMATCH!>{ <!CANNOT_INFER_PARAMETER_TYPE!>x<!> -> "x" }<!>
}
...
fun test_3(b: Boolean) {
    konst x1: String.(String) -> String = if (b) {
        { str, <!CANNOT_INFER_PARAMETER_TYPE!>str2<!> -> "this" }
    } else {
        { str, <!CANNOT_INFER_PARAMETER_TYPE!>str2<!> -> "this" }
    }

    konst x2: String.() -> String = if (b) {
        { <!UNRESOLVED_REFERENCE!>it<!> }
    } else {
        { <!UNRESOLVED_REFERENCE!>it<!> }
    }

    konst x3: String.() -> String = if (b) {
