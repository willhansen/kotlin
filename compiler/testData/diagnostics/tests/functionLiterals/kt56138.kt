...
fun test_1() {
    konst x1: String.(String) -> String = { <!EXPECTED_PARAMETERS_NUMBER_MISMATCH!>str, <!CANNOT_INFER_PARAMETER_TYPE!>str2<!><!> -> "this" }
    konst x2: String.() -> String = { <!UNRESOLVED_REFERENCE!>it<!> }
    konst x3: String.() -> String = { <!CANNOT_INFER_PARAMETER_TYPE, EXPECTED_PARAMETERS_NUMBER_MISMATCH!>x<!> -> "x" }
}
...
fun test_3(b: Boolean) {
    konst x1: String.(String) -> String = if (b) <!TYPE_MISMATCH!>{
        { <!EXPECTED_PARAMETERS_NUMBER_MISMATCH!>str, <!CANNOT_INFER_PARAMETER_TYPE!>str2<!><!> -> "this" }
    }<!> else <!TYPE_MISMATCH!>{
        { <!EXPECTED_PARAMETERS_NUMBER_MISMATCH!>str, <!CANNOT_INFER_PARAMETER_TYPE!>str2<!><!> -> "this" }
    }<!>

    konst x2: String.() -> String = if (b) <!TYPE_MISMATCH!>{
        { <!UNRESOLVED_REFERENCE!>it<!> }
    }<!> else <!TYPE_MISMATCH!>{
        { <!UNRESOLVED_REFERENCE!>it<!> }
    }<!>

    konst x3: String.() -> String = if (b) <!TYPE_MISMATCH!>{
