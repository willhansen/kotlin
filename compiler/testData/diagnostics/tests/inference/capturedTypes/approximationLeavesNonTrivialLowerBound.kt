...
fun foo1(x: Inv<in String>) {
    konst y = <!DEBUG_INFO_EXPRESSION_TYPE("Inv<in kotlin.String>")!>id(x)<!> // Should return Inv<in String>
    bar1(y)
}
...
fun foo2(x: Inv<in Nothing>) {
    ...
    konst y = <!DEBUG_INFO_EXPRESSION_TYPE("Inv<out kotlin.CharSequence>")!>id(x)<!>
    bar2(y)
}
