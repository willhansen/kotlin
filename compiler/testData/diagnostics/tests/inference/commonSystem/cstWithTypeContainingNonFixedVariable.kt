...
fun testVariableWithBound() {
    ...
    konst c2 = <!TYPE_MISMATCH!>select<!>(SubInv<String>(), <!TYPE_MISMATCH!>createWithNumberBound<!>())

    <!DEBUG_INFO_EXPRESSION_TYPE("Inv<kotlin.String>")!>c2<!>

    konst c3 = <!TYPE_MISMATCH!>select<!>(SubInv<Double>(), <!TYPE_MISMATCH!>createWithIntBound<!>())
...
fun testCapturedVariable() {
    ...
    konst c3 = <!TYPE_MISMATCH!>select<!>(SubInv<Number>(), createInvIn())
