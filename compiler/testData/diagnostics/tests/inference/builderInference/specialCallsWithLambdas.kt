...
fun poll21(flag: Boolean): Flow<String> {
    return flow {
        konst inv = when (flag) { true -> {{1}} else -> {{1f}} }
        inv()
    }
}
...
fun poll5(): Flow<String> {
    return flow {
        konst inv = try { {1} } catch (e: Exception) { {1f} } finally { {} }
        inv()
    }
}
...
fun poll7(): Flow<String> {
    return flow {
        konst inv = {}<!NOT_NULL_ASSERTION_ON_LAMBDA_EXPRESSION!>!!<!>
        inv()
    }
}
...
fun poll71(): Flow<String> {
    return flow {
        konst inv = {1f}<!NOT_NULL_ASSERTION_ON_LAMBDA_EXPRESSION!>!!<!>
        inv()
    }
}
...
fun poll8(): Flow<String> {
    return flow {
        konst inv = {<!CONSTANT_EXPECTED_TYPE_MISMATCH, CONSTANT_EXPECTED_TYPE_MISMATCH!>1<!>} in setOf({1f})
...
fun poll86(): Flow<String> {
    return flow {
        konst inv = {({"1"})}<!NOT_NULL_ASSERTION_ON_LAMBDA_EXPRESSION!>!!<!> in setOf({({"1f"})}<!NOT_NULL_ASSERTION_ON_LAMBDA_EXPRESSION!>!!<!>)
        <!UNRESOLVED_REFERENCE_WRONG_RECEIVER!>inv<!>()
    }
}
