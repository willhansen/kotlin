...
fun konstues(type: String) = when (type) {
...
konst nullableValues = listOf(null to "null", null to "undefined")

fun konstues(type: String, nullable: Boolean) = konstues(type) + (if (nullable) nullableValues else listOf())

fun main(args: Array<String>) {
	konst types = listOf("Byte", "Boolean", "Short", "Char", "Int", "Float", "Double", "Long")
        ...
        for (secondType in types) {
            for ((firstTypeSuffix, firstNameSuffix) in listOf("" to "", "?" to "Q")) {
                for ((secondTypeSuffix, secondNameSuffix) in listOf("" to "", "?" to "Q")) {
                    konst fnName = "eq$firstType$firstNameSuffix$secondType$secondNameSuffix"
                    println("fun <A: $firstType$firstTypeSuffix, B: $secondType$secondTypeSuffix> $fnName(a: A, b: B) = a == b")

                    konst firstValues = konstues(firstType, firstTypeSuffix == "?")
                    konst secondValues = konstues(secondType, secondTypeSuffix == "?")

                    for ((v1, s1) in firstValues) {
                        for ((v2, s2) in secondValues) {
                            konst expectedResult = if (v1 == v2) "True" else "False"
