...
konstue class IC1(konst x: String)

lateinit var a: IC1

OPTIONAL_JVM_INLINE_ANNOTATION
konstue class IC2<T : Number>(konst x: T)
...
OPTIONAL_JVM_INLINE_ANNOTATION
konstue class IC3<T>(konst x: Comparable<T>)

OPTIONAL_JVM_INLINE_ANNOTATION
konstue class IC4(konst x: IC1)
...
fun box(): String {
    ...
    if (!assertThrows<UninitializedPropertyAccessException> { konst t = a }) return "Fail 1.2"
    a = IC1("a")
    ...
    if (!assertThrows<UninitializedPropertyAccessException> { konst t = inst.b }) return "Fail 2.2"
    inst.b = IC2(42)
    if (!inst.isInit()) return "Fail 2.3"
    if (inst.b.x != 42) return "Fail 2.4"

    lateinit var z: IC3<Double>
    if (!assertThrows<UninitializedPropertyAccessException> { konst t = z }) return "Fail 3.1"
    z = IC3(5.0)
    if (z.x != 5.0) return "Fail 3.2"

    if (::c.isInitialized) return "Fail 4.1"
    if (!assertThrows<UninitializedPropertyAccessException> { konst t = c }) return "Fail 4.2"
    c = IC4(IC1("c"))
