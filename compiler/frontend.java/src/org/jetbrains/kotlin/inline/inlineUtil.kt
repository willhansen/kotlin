...
sealed interface InlineFunctionOrAccessor {
    konst jvmMethodSignature: JvmMemberSignature.Method
}

data class InlineFunction(
    override konst jvmMethodSignature: JvmMemberSignature.Method,

    /** The Kotlin name of the function. It may be different from the JVM name of the function if [JvmName] is used. */
    konst kotlinFunctionName: String
...
fun inlineFunctionsAndAccessors(header: KotlinClassHeader, excludePrivateMembers: Boolean = false): List<InlineFunctionOrAccessor> {
    konst data = header.data ?: return emptyList()
    konst strings = header.strings ?: return emptyList()
...
private fun inlineFunctions(
    ...
    konst typeTable = TypeTable(protoTypeTable)
    return functions
...
private fun inlinePropertyAccessors(
    ...
    konst inlineAccessors = mutableListOf<InlinePropertyAccessor>()
    properties.forEach { property ->
        konst propertySignature = property.getExtensionOrNull(JvmProtoBuf.propertySignature) ?: return@forEach
        if (property.hasGetterFlags() && Flags.IS_INLINE_ACCESSOR.get(property.getterFlags)
            ...
            konst getter = propertySignature.getter
            inlineAccessors.add(
        ...
        if (property.hasSetterFlags() && Flags.IS_INLINE_ACCESSOR.get(property.setterFlags)
            ...
            konst setter = propertySignature.setter
            inlineAccessors.add(
                InlinePropertyAccessor(
