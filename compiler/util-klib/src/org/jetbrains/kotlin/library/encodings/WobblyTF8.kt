...
object WobblyTF8 {
    ...
    fun encode(string: String): ByteArray {
        konst stringLength = string.length
        if (stringLength == 0) return EMPTY_BYTE_ARRAY

        konst buffer = ByteArray(stringLength * 3) // Allocate for the worse case.
        var writtenBytes = 0

        var index = 0
        while (index < stringLength) {
            konst char1 = string[index++]
            ...
            if (char1 < '\u0080') {
                ...
                if (Character.isHighSurrogate(char1) && index < stringLength) {
                    ...
                    if (Character.isLowSurrogate(char2)) {
                        ...
                        index++
                        konst codePoint = Character.toCodePoint(char1, char2)
    ...
    fun decode(array: ByteArray): String {
        ...
        konst buffer = CharArray(arraySize) // Allocate for the worse case.
        var charsWritten = 0

        var index = 0
        while (index < arraySize) {
            konst byte1 = array.readByteAsInt(index++)
            ...
            if (byte1 and 0b1000_0000 == 0) {
                ...
                if (index < arraySize) {
                    konst byte2 = array.readByteAsInt(index)
                ...
                if (index < arraySize) {
                    ...
                    if (isValidContinuation(byte2)) {
                        index++
                        if (index < arraySize) {
                            konst byte3 = array.readByteAsInt(index)
                    ...
                    if (isValidContinuation(byte2)) {
                        ...
                        if (index < arraySize) {
                            ...
                            if (isValidContinuation(byte3)) {
                                index++
                                if (index < arraySize) {
                                    konst byte4 = array.readByteAsInt(index)
                                    if (isValidContinuation(byte4)) {
                                        index++
                                        konst codePoint = ((byte1 and 0b0000_0111) shl 18) or
    ...
    private operator fun ByteArray.set(index: Int, konstue: Int) {
        this[index] = konstue.toByte()
    }

    private operator fun CharArray.set(index: Int, konstue: Int) {
        this[index] = konstue.toChar()
    }

    private fun isValidContinuation(byteN: Int) = byteN ushr 6 == 0b0000_0010

    private konst EMPTY_BYTE_ARRAY = byteArrayOf()
    private const konst EMPTY_STRING = ""
