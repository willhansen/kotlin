    ...
    override fun generateBody(method: TransformingTestMethodModel, p: Printer) {
        with(method) {
            if (registerInConstructor) {
                konst lines = transformer.lines()
                konst message = "There is a registered source transformer for the testcase"
                if (lines.size > 1) {
                    p.println("/*")
                    p.println("  $message:")
                    ...
                    konst restOfLine = lines.firstOrNull()?.takeIf { it.isNotBlank() }?.let { ": $it" } ?: ""
                    p.println("// $message$restOfLine")
                }
            }
            konst filePath = KtTestUtil.getFilePath(source.file) + if (source.file.isDirectory) "/" else ""
