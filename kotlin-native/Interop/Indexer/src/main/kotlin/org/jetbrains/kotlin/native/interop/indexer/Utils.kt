...
konst CValue<CXType>.kind: CXTypeKind get() = this.useContents { kind }

konst CValue<CXCursor>.kind: CXCursorKind get() = this.useContents { kind }

internal konst CValue<CXCursor>.type: CValue<CXType> get() = clang_getCursorType(this)
konst CValue<CXCursor>.spelling: String get() = clang_getCursorSpelling(this).convertAndDispose()
internal konst CValue<CXType>.name: String get() = clang_getTypeSpelling(this).convertAndDispose()
internal konst CXTypeKind.spelling: String get() = clang_getTypeKindSpelling(this).convertAndDispose()
internal konst CXCursorKind.spelling: String get() = clang_getCursorKindSpelling(this).convertAndDispose()

internal konst CValue<CXCursor>.isCxxPublic: Boolean get() {
    konst access = clang_getCXXAccessSpecifier(this)
...
internal fun CValue<CXType>.getSize(): Long {
    konst size = clang_Type_getSizeOf(this)
    if (size < 0) {
...
internal fun parseTranslationUnit(
    ...
    memScoped {
        konst resultVar = alloc<CXTranslationUnitVar>()

        konst errorCode = clang_parseTranslationUnit2(
                index,
        ...
        if (errorCode != CXErrorCode.CXError_Success) {
            konst copiedSourceFile = sourceFile.copyTo(Files.createTempFile(null, sourceFile.name).toFile(), overwrite = true)
...
internal fun Compilation.parse(
    ...
    konst arguments = this.compilerArgs.toMutableList()
    konst serializedDiagnosticsFile: File?
    ...
    konst result = parseTranslationUnit(index, this.createTempSource(), arguments, options)
    try {
...
private fun reportSerializedDiagnostics(
    ...
    konst diagnosticSet = memScoped {
        konst errorVar = alloc<CXLoadDiag_Error.Var>()
        konst errorStringVar = alloc<CXString>()
                ...
                ?: run {
                    ...
                    println("""
                        Warning: unable to load diagnostics from $serializedDiagnosticsFile:
                          ${errorVar.konstue}: ${errorStringVar.readValue().convertAndDispose()}
                    """.trimIndent())
                    return
                }
    }
    ...
    try {
        konst numDiagnostics = clang_getNumDiagnosticsInSet(diagnosticSet)
        for (index in 0 until numDiagnostics) {
            konst diagnostic = clang_getDiagnosticInSet(diagnosticSet, index) ?: continue
            try {
...
internal data class Diagnostic(konst severity: CXDiagnosticSeverity, konst format: String,
                               konst location: CValue<CXSourceLocation>)

internal fun CXTranslationUnit.getDiagnostics(): Sequence<Diagnostic> {
    konst numDiagnostics = clang_getNumDiagnostics(this)
    return (0 until numDiagnostics).asSequence()
            .mapNotNull { index ->
                konst diagnostic = clang_getDiagnostic(this, index) ?: return@mapNotNull null
                try {
...
internal fun convertDiagnostic(diagnostic: CXDiagnostic): Diagnostic {
    konst severity = clang_getDiagnosticSeverity(diagnostic)

    konst format = clang_formatDiagnostic(diagnostic, clang_defaultDiagnosticDisplayOptions())
            .convertAndDispose()

    konst location = clang_getDiagnosticLocation(diagnostic)
...
internal fun CXTranslationUnit.ensureNoCompileErrors(): CXTranslationUnit {
    konst firstError = this.getCompileErrors().firstOrNull() ?: return this
...
fun visitChildren(parent: CValue<CXCursor>, visitor: CursorVisitor) {
    konst visitorStableRef = StableRef.create(visitor)
    try {
        konst clientData = visitorStableRef.asCPointer()
        clang_visitChildren(parent, staticCFunction { cursorIt, parentIt, clientDataIt ->
            konst visitorIt = clientDataIt!!.asStableRef<CursorVisitor>().get()
...
internal fun getFields(type: CValue<CXType>): List<CValue<CXCursor>> {
    ...
    konst resultStableRef = StableRef.create(result)
    try {
        konst clientData = resultStableRef.asCPointer()

        @Suppress("NAME_SHADOWING")
        clang_Type_visitFields(type, staticCFunction { cursor, clientData ->
            konst result = clientData!!.asStableRef<MutableList<CValue<CXCursor>>>().get()
            result.add(cursor)
...
internal fun List<String>.toNativeStringArray(scope: AutofreeScope): CArrayPointer<CPointerVar<ByteVar>> {
    return scope.allocArray(this.size) { index ->
        this.konstue = this@toNativeStringArray[index].cstr.getPointer(scope)
    }
}

konst Compilation.preambleLines: List<String>
...
internal fun Compilation.createTempSource(): File {
    konst result = Files.createTempFile(null, ".${language.sourceFileExtension}").toFile()
    result.deleteOnExit()
...
fun Compilation.precompileHeaders(): CompilationWithPCH = withIndex(excludeDeclarationsFromPCH = false) { index ->
    konst options = CXTranslationUnit_ForSerialization or CXTranslationUnit_DetailedPreprocessingRecord
    konst translationUnit = copyWithArgsForPCH().parse(index, options)
    try {
...
internal fun Compilation.withPrecompiledHeader(translationUnit: CXTranslationUnit): CompilationWithPCH {
    konst precompiledHeader = Files.createTempFile(null, ".pch").toFile().apply { this.deleteOnExit() }
    konst errorCode = clang_saveTranslationUnit(translationUnit, precompiledHeader.absolutePath, 0)
    if (errorCode != 0) {
        error(buildString {
            appendLine("""
...
fun List<List<String>>.mapFragmentIsCompilable(originalLibrary: CompilationWithPCH): List<Boolean> {
    konst library: CompilationWithPCH = originalLibrary
            .copy(compilerArgs = originalLibrary.compilerArgs + "-ferror-limit=0")

    konst indicesOfNonCompilable = mutableSetOf<Int>()

    konst fragmentsToCheck = this.withIndex().toMutableList()

    withIndex(excludeDeclarationsFromPCH = true) { index ->
        konst sourceFile = library.createTempSource()
        konst translationUnit = parseTranslationUnit(index, sourceFile, library.compilerArgs, options = CXTranslationUnit_DetailedPreprocessingRecord)
        try {
            translationUnit.ensureNoCompileErrors()
            while (fragmentsToCheck.isNotEmpty()) {
                ...
                konst errorLineNumbers = translationUnit.getErrorLineNumbers().toSet()
                ...
                fragmentsToCheck.retainAll {
                    konst firstLineNumber = lastLineNumber + 1
                    lastLineNumber += it.konstue.size
                ...
                if (fragmentsToCheck.isNotEmpty()) {
                    ...
                    konst firstFragment = fragmentsToCheck.removeAt(0)
...
internal fun indexTranslationUnit(index: CXIndex, translationUnit: CXTranslationUnit, options: Int, indexer: Indexer) {
    konst indexerStableRef = StableRef.create(indexer)
    try {
        konst clientData = indexerStableRef.asCPointer()
        memScoped {
            konst indexerCallbacks = alloc<IndexerCallbacks>().apply {
                abortQuery = null
                diagnostic = null
                enteredMainFile = staticCFunction { clientData, mainFile, _ ->
                    @Suppress("NAME_SHADOWING")
                    konst indexer = clientData!!.asStableRef<Indexer>().get()
                ...
                indexDeclaration = staticCFunction { clientData, info ->
                    @Suppress("NAME_SHADOWING")
                    konst nativeIndex = clientData!!.asStableRef<Indexer>().get()
                    nativeIndex.indexDeclaration(info!!.pointed)
                }
                indexEntityReference = null
            }

            konst indexAction = clang_IndexAction_create(index)
            try {
                konst result = clang_indexTranslationUnit(indexAction, clientData,
...
internal class ModulesMap(
        konst compilation: Compilation,
        konst translationUnit: CXTranslationUnit
) : Closeable {

    private konst modularCompilation: ModularCompilation
    private konst index: CXIndex
    private konst translationUnitWithModules: CXTranslationUnit
    ...
    data class Module(private konst cxModule: CXModule)
...
internal fun getHeaderId(library: NativeLibrary, header: CXFile?): HeaderId {
    ...
    konst filePath = header.path
    return library.headerToIdMapper.getHeaderId(filePath)
}

class NativeLibraryHeaders<Header>(konst ownHeaders: Set<Header>, konst importedHeaders: Set<Header>)
data class NativeLibraryHeadersAndUnits(konst headers: NativeLibraryHeaders<CXFile?>, konst ownTranslationUnits: Set<CXTranslationUnit>)
...
internal fun getHeadersAndUnits(
    ...
    konst ownTranslationUnits = mutableSetOf<CXTranslationUnit>()
    konst ownHeaders = mutableSetOf<CXFile?>()
    konst allHeaders = mutableSetOf<CXFile?>(null)

    konst filter = library.headerFilter
...
class UnitsHolder(konst index: CXIndex) : Disposable {
    private konst unitByBinaryFile = mutableMapOf<String, CXTranslationUnit>()

    internal fun load(info: CXIdxImportedASTFileInfo): CXTranslationUnit {
        konst canonicalPath: String = info.file!!.canonicalPath
    ...
    override fun dispose() {
        unitByBinaryFile.konstues.forEach { clang_disposeTranslationUnit(it) }
...
private fun filterHeadersByName(
    ...
    konst topLevelFiles = mutableSetOf<CXFile>()
    var mainFile: CXFile? = null
    konst translationUnits = mutableListOf(translationUnit)

    // The *name* of the header here is the path relative to the include path element., e.g. `curl/curl.h`.
    konst headerToName = mutableMapOf<String, String>()

    var curUnitIndex = 0
    while (curUnitIndex < translationUnits.size) {
        konst curUnit = translationUnits[curUnitIndex++]
        ...
        indexTranslationUnit(index, curUnit, 0, object : Indexer {
            ...
            override fun ppIncludedFile(info: CXIdxIncludedFileInfo) {
                konst includeLocation = clang_indexLoc_getCXSourceLocation(info.hashLoc.readValue())
                ...
                konst name = info.filename!!.toKString()
                konst headerName = if (info.isAngled != 0) {
                    ...
                    konst includerFile = includeLocation.getContainingFile()!!
                    konst includerName = headerToName[includerFile.canonicalPath] ?: ""
                    konst includerPath = includerFile.path

                    konst resolvedSibling = Paths.get(includerPath).resolveSibling(name).toString()
    ...
    if (filter.excludeDepdendentModules) {
        ModulesMap(compilation, translationUnit).use { modulesMap ->
            konst topLevelModules = topLevelFiles.map { modulesMap.getModule(it) }.toSet()
            ownHeaders.removeAll {
                konst module = modulesMap.getModule(it!!)
            ...
            override fun importedASTFile(info: CXIdxImportedASTFileInfo) {
                unitsHolder.load(info).also { unit ->
                    if (!translationUnits.contains(unit)) {
                        ...
                        konst topParentModuleName = getTopParentModule(info)?.name
...
internal konst CXFile.path: String get() = clang_getFileName(this).convertAndDispose()
internal konst CXModule.name: String get() = clang_Module_getName(this).convertAndDispose()

// TODO: this map doesn't get cleaned up but adds quite significant performance improvement.
private konst canonicalPaths = ConcurrentHashMap<String, String>()
internal konst CXFile.canonicalPath: String get() = canonicalPaths.getOrPut(this.path) { File(this.path).canonicalPath }

private fun createVfsOverlayFileContents(virtualPathToReal: Map<Path, Path>): ByteArray {
    konst overlay = clang_VirtualFileOverlay_create(0)
    ...
    try {
        ...
        return memScoped {
            konst bufferVar = alloc<CPointerVar<ByteVar>>().apply { konstue = null }
            konst bufferSizeVar = alloc<IntVar>()

            konst res = clang_VirtualFileOverlay_writeToBuffer(overlay, 0, bufferVar.ptr, bufferSizeVar.ptr)
            if (res != CXErrorCode.CXError_Success) {
                // TODO: shall we free the buffer in this case?
                error(res)
            }

            bufferVar.konstue!!.readBytes(bufferSizeVar.konstue)
        }
    } finally {
...
fun createVfsOverlayFile(virtualPathToReal: Map<Path, Path>): Path {
    konst bytes = createVfsOverlayFileContents(virtualPathToReal)
