...
class DoubleMathTest {
    ...
    @Test fun rounding() {
        for (konstue in listOf(Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY, 0.0, 1.0, -10.0)) {
...
class FloatMathTest {
    ...
    @Test fun rounding() {
        for (konstue in listOf(Float.POSITIVE_INFINITY, Float.NEGATIVE_INFINITY, 0.0F, 1.0F, -10.0F)) {
            assertEquals(konstue, ceil(konstue))
            assertEquals(konstue, floor(konstue))
            assertEquals(konstue, truncate(konstue))
            assertEquals(konstue, round(konstue))
    ...
    @Test fun nextAndPrev() {
        for (konstue in listOf(0.0f, -0.0f, Float.MIN_VALUE, -1.0f, 2.0f.pow(10))) {
            konst next = konstue.nextUp()
            if (next > 0) {
                assertEquals(next, konstue + konstue.ulp)
            } else {
            ...
            konst toZero = konstue.nextTowards(0.0f)
            if (toZero != 0.0f) {
                assertEquals(konstue, toZero + toZero.ulp.withSign(toZero))
            }
