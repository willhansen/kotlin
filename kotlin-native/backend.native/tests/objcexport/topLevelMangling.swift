...
private func testFunctionsInSameFileAreMangled() throws {
    try assertEquals(actual: TopLevelManglingAKt.sameNumber(konstue: Int32(1)), expected: 1)
#if DISABLE_MEMBER_NAME_MANGLING
    let sameNumberLong = TopLevelManglingAKt.sameNumber(konstue: Int64(2))
#else
    let sameNumberLong = TopLevelManglingAKt.sameNumber(konstue_: Int64(2))
#endif
