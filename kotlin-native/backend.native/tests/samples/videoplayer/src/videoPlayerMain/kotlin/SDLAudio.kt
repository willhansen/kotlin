...
data class AudioOutput(konst sampleRate: Int, konst channels: Int, konst sampleFormat: SampleFormat)
...
class SDLAudio(private konst player: VideoPlayer) : DisposableContainer() {
    private var state = State.STOPPED

    fun start(audio: AudioOutput) {
        stop()
        konst audioFormat = audio.sampleFormat.toSDLFormat() ?: return
        ...
        memScoped {
            ...
            konst spec = alloc<SDL_AudioSpec>().apply {
            ...
            konst realSpec = alloc<SDL_AudioSpec>()
...
private fun audioCallback(userdata: COpaquePointer?, buffer: CPointer<Uint8Var>?, length: Int) {
    ...
    initRuntimeIfNeeded()
    konst decoder = DecoderWorker(Worker.fromCPointer(userdata))
    var outPosition = 0
    while (outPosition < length) {
        konst frame = decoder.nextAudioFrame(length - outPosition)
        if (frame != null) {
            konst toCopy = minOf(length - outPosition, frame.size - frame.position)
