...
class SDLInput(private konst player: VideoPlayer) : DisposableContainer() {
    private konst event = arena.alloc<SDL_Event>().ptr
    ...
    fun check() {
        while (SDL_PollEvent(event.reinterpret()) != 0) {
            when (event.pointed.type) {
                ...
                SDL_KEYDOWN -> {
                    konst keyboardEvent = event.reinterpret<SDL_KeyboardEvent>().pointed
