...
private fun process(function: LLVMValueRef, currentThreadTLV: LLVMValueRef) {
    konst entry = LLVMGetEntryBasicBlock(function) ?: return
    konst load = filterLoads(entry, currentThreadTLV).firstOrNull() ?: return
    getBasicBlocks(function)
            .flatMap { filterLoads(it, currentThreadTLV) }
            .filter { it != load }
            .toList() // to force ekonstuating of all sequences above, because removing something during iteration is bad idea
            .forEach {
...
internal fun removeMultipleThreadDataLoads(context: BitcodePostProcessingContext) {
    konst currentThreadTLV = context.llvm.runtimeAnnotationMap["current_thread_tlv"]?.singleOrNull() ?: return
