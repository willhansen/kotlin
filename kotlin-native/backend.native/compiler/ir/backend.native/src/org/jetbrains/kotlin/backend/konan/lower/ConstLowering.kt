...
internal class ConstLowering(konst context: Context) : FileLoweringPass {
...
private class NativeInlineConstTransformer : InlineConstTransformer() {
    override konst IrField.constantInitializer get() =
        (initializer?.expression as? IrConst<*>)
                ...
                ?.takeUnless { it.kind == IrConstKind.Double && IrConstKind.Double.konstueOf(it).isNaN() }
                ?.takeUnless { it.kind == IrConstKind.Float && IrConstKind.Float.konstueOf(it).isNaN() }
