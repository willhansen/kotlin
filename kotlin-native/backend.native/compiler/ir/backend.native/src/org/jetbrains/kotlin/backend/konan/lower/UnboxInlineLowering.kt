...
private class AccessorInliner(commonBackendContext: CommonBackendContext) : IrElementTransformerVoid() {
    ...
    private konst context = commonBackendContext as Context
    private konst anyType = context.irBuiltIns.anyType
    ...
    private fun tryInlineUnbox(call: IrCall): IrExpression? {
        konst returnClass = call.type.getClass()!!
        konst singleStatement = context.getUnboxFunction(returnClass).body?.statements?.singleOrNull()
        return if (singleStatement is IrReturn) {
            konst retVal = singleStatement.konstue
            if (retVal is IrGetField) {
                // Boxed primitive types (Int, Short,..) have `konstue` field
                // Inline unsigned classes (UInt, UShort,..) have `data` field
                konst field = retVal.symbol.owner
