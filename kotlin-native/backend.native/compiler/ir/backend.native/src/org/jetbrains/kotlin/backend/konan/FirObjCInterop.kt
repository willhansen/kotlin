...
private konst objCDirectClassId = ClassId.topLevel(objCDirectFqName)
private konst objCMethodClassId = ClassId.topLevel(objCMethodFqName)
private konst objCObjectClassId = ClassId.topLevel(objCObjectFqName)
private konst objCFactoryClassId = ClassId.topLevel(objCFactoryFqName)
private konst objCConstructorClassId = ClassId.topLevel(objCConstructorFqName)
private konst externalObjCClassClassId = ClassId.topLevel(externalObjCClassFqName)
...
internal fun FirFunction.getObjCMethodInfoFromOverriddenFunctions(session: FirSession, scopeSession: ScopeSession): ObjCMethodInfo? {
    ...
    return when (konst symbol = this.symbol) {
        is FirNamedFunctionSymbol -> {
            konst firClassSymbol = containingClassLookupTag()?.toSymbol(session) as FirClassSymbol<*>?
            firClassSymbol?.let {
                konst unsubstitutedScope = it.unsubstitutedScope(session, scopeSession, withForcedTypeCalculator = false, memberRequiredPhase = null)
...
private fun FirConstructor.getObjCInitMethod(session: FirSession, scopeSession: ScopeSession): FirFunction? {
    this.annotations.getAnnotationByClassId(objCConstructorClassId, session)?.let { annotation ->
        konst initSelector: String = annotation.constStringArgument("initSelector")
        konst classSymbol = containingClassLookupTag()?.toSymbol(session) as FirClassSymbol<*>
        konst initSelectors = mutableListOf<FirFunction>()
...
private fun FirAnnotation.constStringArgument(argumentName: String): String =
        constArgument(argumentName) as? String ?: error("Expected string constant konstue of argument '$argumentName' at annotation $this")
...
private fun FirAnnotation.constArgument(argumentName: String) =
        (argumentMapping.mapping[Name.identifier(argumentName)] as? FirConstExpression<*>)?.konstue
