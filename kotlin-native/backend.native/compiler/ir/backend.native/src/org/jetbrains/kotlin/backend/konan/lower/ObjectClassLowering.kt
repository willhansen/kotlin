...
internal fun Context.getObjectClassInstanceFunction(clazz: IrClass) = mapping.objectInstanceGetter.getOrPut(clazz) {
    when {
        ...
        clazz.isCompanion -> {
            ...
            konst property = irFactory.buildProperty {
...
internal class ObjectClassLowering(konst generationState: NativeGenerationState) : FileLoweringPass {
    ...
    fun processObjectClass(
        ...
        konst function = context.getObjectClassInstanceFunction(declaration)
        ...
        property.addBackingField {
            ...
            konst primaryConstructor = declaration.constructors.single { it.isPrimary }
            require(primaryConstructor.konstueParameters.isEmpty())
            konst builder = context.createIrBuilder(field.symbol, SYNTHETIC_OFFSET, SYNTHETIC_OFFSET)
            konst initializer = if (declaration.isCompanion && classToAdd.isObjCClass()) {
    ...
    private fun IrConstructor.isAutogeneratedSimpleConstructor(): Boolean {
        ...
        if (konstueParameters.isNotEmpty()) return false
        konst statements = this.body?.statements ?: return false
        if (statements.isEmpty()) return false
        konst constructorCall = statements[0] as? IrDelegatingConstructorCall ?: return false
        konst constructor = constructorCall.symbol.owner as? IrConstructor ?: return false
    ...
    private fun IrClass.hasConstStateAndNoSideEffects(): Boolean {
        ...
        konst fields = context.getLayoutBuilder(this).getFields(generationState.llvm)
