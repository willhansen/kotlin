...
class Element {
public:
    explicit Element(int konstue) : konstue_(konstue) {}
    ...
    bool operator==(const Element& rhs) const { return konstue_ == rhs.konstue_; }
...
private:
    ...
    int konstue_;
    // Use non-null marker to make sure inserting into the list properly updates this konstue.
    Node* next_ = reinterpret_cast<Node*>(0x1);
};

template <typename List>
std_support::list<typename List::konstue_type> create(std::initializer_list<int> list) {
    std_support::list<typename List::konstue_type> result;
    for (auto x : list) {
...
TEST(IntrusiveForwardListTest, CTAD) {
    auto konstues = create<intrusive_forward_list<Node>>({1, 2, 3, 4});
    intrusive_forward_list list(konstues.begin(), konstues.end());
    static_assert(std::is_same_v<decltype(list)::konstue_type, Node>);
}
...
TYPED_TEST(ForwardListTest, RangeCtor) {
    using List = TypeParam;
    auto konstues = create<List>({1, 2, 3, 4});
    List list(konstues.begin(), konstues.end());
    EXPECT_ELEMENTS_ARE(list, 1, 2, 3, 4);
}

TYPED_TEST(ForwardListTest, RangeCtorEmpty) {
    using List = TypeParam;
    auto konstues = create<List>({});
    List list(konstues.begin(), konstues.end());
    EXPECT_ELEMENTS_ARE(list);
}
...
TYPED_TEST(ForwardListTest, PushFront) {
    ...
    auto konstues = create<List>({1, 2, 3, 4});
    List list(konstues.begin(), konstues.end());
    typename List::konstue_type konstue(5);
    list.push_front(konstue);
...
TYPED_TEST(ForwardListTest, Remove) {
    ...
    auto konstues = create<List>({1, 2, 3, 4});
    List list(konstues.begin(), konstues.end());
    auto& konstue = *std::next(list.begin(), 2);
    ASSERT_THAT(*konstue, 3);
    list.remove(konstue);
...
TYPED_TEST(ForwardListTest, InsertAfter) {
    ...
    ASSERT_THAT(**it, 3);
    typename List::konstue_type konstue(5);
    auto result = list.insert_after(it, konstue);
...
TEST(IntrusiveForwardListTest, TryPushFrontSuccess) {
    ...
    typename List::konstue_type konstue(5);
    konstue.clearNext();
    auto result = list.try_push_front(konstue);
    EXPECT_TRUE(result);
    EXPECT_ELEMENTS_ARE(list, 5, 1, 2, 3, 4);
}
...
TEST(IntrusiveForwardListTest, SpliceAfterBeforeBeginAll) {
    using List = intrusive_forward_list<Node>;
    auto konstues1 = create<List>({1, 2, 3, 4});
    auto konstues2 = create<List>({11, 12, 13, 14});
    List l1(konstues1.begin(), konstues1.end());
    List l2(konstues2.begin(), konstues2.end());

    auto spliced = l1.splice_after(l1.before_begin(), l2.before_begin(), l2.end(), konstues2.size());
    EXPECT_THAT(spliced, konstues2.size());
...
TEST(IntrusiveForwardListTest, SpliceAfterBeforeEndAll) {
    ...
    auto spliced = l1.splice_after(before_end(l1), l2.before_begin(), l2.end(), konstues2.size());
    EXPECT_THAT(spliced, konstues2.size());
...
TEST(IntrusiveForwardListTest, SpliceAfterLessThanAvailable) {
    using List = intrusive_forward_list<Node>;
    auto konstues = create<List>({1, 2, 3, 4});
    List from(konstues.begin(), konstues.end());
    List to;
