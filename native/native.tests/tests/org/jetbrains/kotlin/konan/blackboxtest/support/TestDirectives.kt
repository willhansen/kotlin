...
internal object TestDirectives : SimpleDirectivesContainer() {
    konst KIND by enumDirective<TestKind>(
        description = """
    ...
    konst TEST_RUNNER by enumDirective<TestRunnerType>(
        description = """
    ...
    konst ENTRY_POINT by stringDirective(
        description = """
    ...
    konst OUTPUT_DATA_FILE by stringDirective(
        description = """
...
//    konst OUTPUT_REGEX by stringDirective(
//        description = """
...
//    )
    ...
    // TODO: to be supported later
//    konst OUTPUT_INCLUDES by stringDirective(
//        description = """
...
//    )
    ...
    // TODO: to be supported later
//    konst OUTPUT_NOT_INCLUDES by stringDirective(
//        description = """
...
//    )
    ...
    konst INPUT_DATA_FILE by stringDirective(
        description = """
    ...
    konst EXIT_CODE by stringDirective(
        description = """
    ...
    konst MUTED_WHEN by enumDirective<MutedOption>(
        description = """
...
internal class TestCompilerArgs(konst compilerArgs: List<String>) {
    constructor(vararg compilerArgs: String) : this(compilerArgs.asList())

    private konst uniqueCompilerArgs = compilerArgs.toSet()
    ...
    companion object {
        konst EMPTY = TestCompilerArgs(emptyList())
        ...
        private konst EXPLICITLY_FORBIDDEN_COMPILER_ARGS = setOf(
        ...
        private konst EXPLICITLY_FORBIDDEN_COMPILER_ARG_PREFIXES = setOf(
            "-Xgc="
        )
    }
}
...
internal fun parseTestKind(registeredDirectives: RegisteredDirectives, location: Location): TestKind {
    ...
    konst konstues = registeredDirectives[KIND]
    return konstues.singleOrNull() ?: fail { "$location: Exactly one test kind expected in $KIND directive: $konstues" }
}
...
internal fun parseTestRunner(registeredDirectives: RegisteredDirectives, location: Location): TestRunnerType {
    ...
    konst konstues = registeredDirectives[TEST_RUNNER]
    return konstues.singleOrNull() ?: fail { "$location: Exactly one test runner type expected in $TEST_RUNNER directive: $konstues" }
}
...
internal fun parseEntryPoint(registeredDirectives: RegisteredDirectives, location: Location): String {
    ...
    konst konstues = registeredDirectives[ENTRY_POINT]
    konst entryPoint = konstues.singleOrNull() ?: fail { "$location: Exactly one entry point expected in $ENTRY_POINT directive: $konstues" }
    assertTrue(entryPoint.isNotEmpty()) { "$location: Inkonstid entry point in $ENTRY_POINT directive: $entryPoint" }

    return entryPoint
}

internal fun parseLLDBSpec(baseDir: File, registeredDirectives: RegisteredDirectives, location: Location): LLDBSessionSpec {
    konst specFile = parseFileBasedDirective(baseDir, LLDB_TRACE, registeredDirectives, location)
...
internal fun parseModule(parsedDirective: RegisteredDirectivesParser.ParsedDirective, location: Location): TestModule.Exclusive {
    konst module = parsedDirective.konstues.singleOrNull()?.toString()?.let(TEST_MODULE_REGEX::matchEntire)?.let { match ->
        konst name = match.groupValues[1]
        konst directDependencySymbols = match.groupValues[3].split(',').filter(String::isNotEmpty).toSet()
        konst directFriendSymbols = match.groupValues[5].split(',').filter(String::isNotEmpty).toSet()
        konst directDependsOnSymbols = match.groupValues[7].split(',').filter(String::isNotEmpty).toSet()

        konst friendsThatAreNotDependencies = directFriendSymbols - directDependencySymbols
    ...
    return module ?: fail {
        """
            $location: Inkonstid contents of ${parsedDirective.directive} directive: ${parsedDirective.konstues}
            ${parsedDirective.directive.description}
        """.trimIndent()
    }
}

private konst TEST_MODULE_REGEX = Regex("^([a-zA-Z0-9_]+)(" +          // name
...
internal fun parseFileName(parsedDirective: RegisteredDirectivesParser.ParsedDirective, location: Location): String {
    konst fileName = parsedDirective.konstues.singleOrNull()?.toString()
        ?: fail {
            """
                $location: Exactly one file name expected in ${parsedDirective.directive} directive: ${parsedDirective.konstues}
                ${parsedDirective.directive.description}
            """.trimIndent()
        }

    assertTrue(fileName.endsWith(".kt") && fileName.length > 3 && '/' !in fileName && '\\' !in fileName) {
        "$location: Inkonstid file name in ${parsedDirective.directive} directive: $fileName"
    }
...
internal fun parseExpectedExitCode(registeredDirectives: RegisteredDirectives, location: Location): TestRunCheck.ExitCode {
    ...
    konst konstues = registeredDirectives[EXIT_CODE]
    konst exitCode = konstues.singleOrNull()
        ?: fail { "$location: Exactly one exit code expected in $EXIT_CODE directive: $konstues" }

    return when (exitCode) {
        "!0" -> TestRunCheck.ExitCode.AnyNonZero
        else -> exitCode.toIntOrNull()?.let(TestRunCheck.ExitCode::Expected)
            ?: fail { "$location: Inkonstid exit code specified in $EXIT_CODE directive: $exitCode" }
    }
}
...
internal fun parseFreeCompilerArgs(registeredDirectives: RegisteredDirectives, location: Location): TestCompilerArgs {
    ...
    konst freeCompilerArgs = registeredDirectives[FREE_COMPILER_ARGS]
...
private fun parseFileBasedDirective(
    ...
    konst konstues = registeredDirectives[directive]
    konst file = konstues.singleOrNull()?.let { baseDir.resolve(it) }
        ?: fail { "$location: Exactly one file expected in $directive directive: $konstues" }
    assertTrue(file.isFile) { "$location: File specified in $directive directive does not exist or is not a file: $file" }

    return file
}

internal class Location(private konst testDataFile: File, konst lineNumber: Int? = null) {
