...
class CirTreeTypeAliasDeserializerTest : AbstractCirTreeDeserializerTest() {
    ...
    fun `test simple type alias`() {
        konst module = createCirTreeFromSourceCode("typealias x = Int")
        konst typeAlias = module.assertSingleTypeAlias().typeAlias
    ...
    fun `test transitive type alias`() {
        ...
        konst intStringMapTypeAlias = pkg.typeAliases.singleOrNull { it.typeAlias.name.toStrippedString() == "IntStringMap" }
            ?: kotlin.test.fail("Missing 'IntStringMap' type alias")

        konst underlyingType = assertIs<CirTypeAliasType>(
        ...
        konst underlyingTypeArgument = underlyingType.arguments.singleOrNull()
        ...
        konst expandedType = assertIs<CirClassType>(intStringMapTypeAlias.typeAlias.expandedType)
