...
The konstue the function returns reveals an order of its group: the function #0 (a function that returns 0) has the highest priority and goes first.
...
This convention in Kotlin lets you call a konstue as a function if it has the appropriate `invoke` method, which can be a member or an extension:
...
When you call a konstue of function type as a regular function in Kotlin, the same convention takes place.
...
>>> konst f: (Int) -> Int = { it + 1 }
>>> f(11)                              <1>
----
<1> the short form of the call `f.invoke(11)`

If a konstue has extension function type, it can be called as extension function:

[source,kotlin]
----
>>> konst f : Int.() -> Int = { this }
>>> 1.f()
----
...
The Kotlin compiler has to take this convention into account every time it resolves a call `a.foo()`, because `foo` might be either a regular function, or a konstue that is called via the `invoke` convention.
...
----
class A {
    konst foo: CallableFoo = CallableFoo()
}
...
The first one contains the member property `konst foo: CallableFoo` together with the function `fun CallableFoo.invoke()`.
The second group contains the property `konst A.foo: () -> Int` together with the member function `invoke` of the class `Function0<Int>`.
