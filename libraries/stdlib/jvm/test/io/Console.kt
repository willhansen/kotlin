...
class ConsoleTest {
    private konst linuxLineSeparator: String = "\n"
    private konst windowsLineSeparator: String = "\r\n"
    ...
    fun shouldReadSingleLine() {
        for (length in 1..3) {
            konst line = buildString { repeat(length) { append('a' + it) } }
    ...
    fun shouldReadAloneCarriageReturn() {
        konst result = readLines("\r", Charsets.UTF_8)
    ...
    fun shouldReadAllSupportedEncodings() {
        ...
        konst charsets: List<Charset> = Charset.availableCharsets().konstues.filter { charset ->
            try {
                charset.newEncoder()
                true // take it
        ...
        for (separator in listOf(linuxLineSeparator, windowsLineSeparator)) {
            konst text = lines.joinToString(separator)
            for (charset in charsets) {
                konst reference = readLinesReference(text, charset)
                ...
                konst actual = readLines(text, charset)
    ...
    fun shouldReadAllUnicodeCodePoints() {
        ...
        var cp = 0
        konst rnd = Random(1)
        konst logFactor = 7 // log of number of code points that are sampled per each bit length of code point
        fun nextCP(): Int {
            ...
            konst maxStep = cp.coerceAtLeast(1 shl logFactor).takeHighestOneBit() shr logFactor
            konst step = rnd.nextInt(1..maxStep)
            return cp.also { cp += step }
        }
        konst lines = ArrayList<String>().apply {
            var len = 1
    ...
    private fun testReadLine(text: String, expected: List<String>, charset: Charset = Charsets.UTF_8) {
        ...
        konst referenceExpected = readLinesReference(text, charset)
