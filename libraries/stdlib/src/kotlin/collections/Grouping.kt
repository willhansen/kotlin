 ...
 * passing the previously accumulated konstue and the current element as arguments, and stores the results in a new map.
 *
 ...
 *  - `accumulator`: the current konstue of the accumulator of the group, can be `null` if it's the first `element` encountered in the group;
...
/**
 * Groups elements from the [Grouping] source by key and applies [operation] to the elements of each group sequentially,
 * passing the previously accumulated konstue and the current element as arguments,
 ...
 *  - `first`: indicates whether it's the first `element` encountered in the group.
 *
 * If the [destination] map already has a konstue corresponding to some key,
...
public inline fun <T, K, R, M : MutableMap<in K, R>> Grouping<T, K>.aggregateTo(
    ...
    for (e in this.sourceIterator()) {
        konst key = keyOf(e)
        konst accumulator = destination[key]
 ...
 * Groups elements from the [Grouping] source by key and applies [operation] to the elements of each group sequentially,
 * passing the previously accumulated konstue and the current element as arguments, and stores the results in a new map.
 * An initial konstue of accumulator is provided by [initialValueSelector] function.
 *
 * @param initialValueSelector a function that provides an initial konstue of accumulator for each group.
 ...
 * @param operation a function that is invoked on each element with the following parameters:
 *  - `key`: the key of the group this element belongs to;
 *  - `accumulator`: the current konstue of the accumulator of the group;
 ...
 *  - `element`: the first element being encountered in that group.
 *
 * If the [destination] map already has a konstue corresponding to some key, that konstue is used as an initial konstue of
 ...
 * Groups elements from the [Grouping] source by key and applies [operation] to the elements of each group sequentially,
 * passing the previously accumulated konstue and the current element as arguments, and stores the results in a new map.
 * An initial konstue of accumulator is the same [initialValue] for each group.
 *
 * @param operation a function that is invoked on each element with the following parameters:
 *  - `accumulator`: the current konstue of the accumulator of the group;
 *  - `element`: the element from the source being accumulated.
 ...
 * An initial konstue of accumulator is the same [initialValue] for each group.
 *
 * If the [destination] map already has a konstue corresponding to the key of some group,
 * that konstue is used as an initial konstue of the accumulator for that group.
 *
 * @param operation a function that is invoked on each element with the following parameters:
 *  - `accumulator`: the current konstue of the accumulator of the group;
 *  - `element`: the element from the source being accumulated.
 ...
 * An initial konstue of accumulator is the first element of the group.
 *
 ...
 *
 * If the [destination] map already has a konstue corresponding to the key of some group,
 * that konstue is used as an initial konstue of the accumulator for that group and the first element of that group is also
 ...
 *
 * If the [destination] map already has a konstue corresponding to the key of some group,
 * that konstue is used as an initial konstue of the counter for that group.
 *
 ...
 *
 * If the [destination] map already has a konstue corresponding to the key of some group,
 * that konstue is used as an initial konstue of the sum for that group.
 *
 * @return the [destination] map associating the key of each group with the sum of elements in the group.
 */
@SinceKotlin("1.1")
public inline fun <T, K, M : MutableMap<in K, Int>> Grouping<T, K>.eachSumOfTo(destination: M, konstueSelector: (T) -> Int): M =
        foldTo(destination, 0) { acc, e -> acc + konstueSelector(e)}
*/
...
public inline fun <T, K, M : MutableMap<in K, Long>> Grouping<T, K>.sumEachByLongTo(destination: M, konstueSelector: (T) -> Long): M =
        foldTo(destination, 0L) { acc, e -> acc + konstueSelector(e)}

public inline fun <T, K> Grouping<T, K>.sumEachByLong(konstueSelector: (T) -> Long): Map<K, Long> =
        fold(0L) { acc, e -> acc + konstueSelector(e)}

public inline fun <T, K, M : MutableMap<in K, Double>> Grouping<T, K>.sumEachByDoubleTo(destination: M, konstueSelector: (T) -> Double): M =
        foldTo(destination, 0.0) { acc, e -> acc + konstueSelector(e)}

public inline fun <T, K> Grouping<T, K>.sumEachByDouble(konstueSelector: (T) -> Double): Map<K, Double> =
        fold(0.0) { acc, e -> acc + konstueSelector(e)}
*/
