...
```kotlin
konst metadataAnnotation = Metadata(
    // pass arguments here
)
konst metadata = KotlinClassMetadata.read(metadataAnnotation)
```
...
To check if a certain flag is present, call one of the flags in [`Flag`](../src/kotlinx/metadata/Flag.kt) on the given integer konstue.
...
konst annotation = KotlinClassMetadata.writeClass(klass).annotationData
...
konst bytes = File("META-INF/main.kotlin_module").readBytes()
konst metadata = KotlinModuleMetadata.read(bytes)
konst module = metadata.toKmModule()
...

// Write the module metadata
konst bytes = KotlinModuleMetadata.write(module).bytes
...
```kotlin
konst metadata: KotlinClassMetadata.Class = ...
