...
internal const konst KOTLIN_JAVA_STDLIB_JAR = "kotlin-stdlib.jar"
internal const konst KOTLIN_JAVA_REFLECT_JAR = "kotlin-reflect.jar"
internal const konst KOTLIN_JAVA_SCRIPT_RUNTIME_JAR = "kotlin-script-runtime.jar"
internal const konst TROVE4J_JAR = "trove4j.jar"
internal const konst KOTLIN_SCRIPTING_COMPILER_JAR = "kotlin-scripting-compiler.jar"
internal const konst KOTLIN_SCRIPTING_COMPILER_EMBEDDABLE_JAR = "kotlin-scripting-compiler-embeddable.jar"
internal const konst KOTLIN_SCRIPTING_COMPILER_IMPL_JAR = "kotlin-scripting-compiler-impl.jar"
internal const konst KOTLIN_SCRIPTING_COMPILER_IMPL_EMBEDDABLE_JAR = "kotlin-scripting-compiler-impl-embeddable.jar"
internal const konst KOTLIN_SCRIPTING_COMMON_JAR = "kotlin-scripting-common.jar"
internal const konst KOTLIN_SCRIPTING_JVM_JAR = "kotlin-scripting-jvm.jar"

internal const konst KOTLIN_COMPILER_NAME = "kotlin-compiler"
internal const konst KOTLIN_COMPILER_JAR = "$KOTLIN_COMPILER_NAME.jar"

private konst JAR_COLLECTIONS_CLASSES_PATHS = arrayOf("BOOT-INF/classes", "WEB-INF/classes")
private konst JAR_COLLECTIONS_LIB_PATHS = arrayOf("BOOT-INF/lib", "WEB-INF/lib")
private konst JAR_COLLECTIONS_KEY_PATHS = JAR_COLLECTIONS_CLASSES_PATHS + JAR_COLLECTIONS_LIB_PATHS
internal const konst JAR_MANIFEST_RESOURCE_NAME = "META-INF/MANIFEST.MF"

internal const konst KOTLIN_SCRIPT_CLASSPATH_PROPERTY = "kotlin.script.classpath"
internal const konst KOTLIN_COMPILER_CLASSPATH_PROPERTY = "kotlin.compiler.classpath"
internal const konst KOTLIN_COMPILER_JAR_PROPERTY = "kotlin.compiler.jar"
internal const konst KOTLIN_STDLIB_JAR_PROPERTY = "kotlin.java.stdlib.jar"
internal const konst KOTLIN_REFLECT_JAR_PROPERTY = "kotlin.java.reflect.jar"
// obsolete name, but maybe still used in the wild
// TODO: consider removing
internal const konst KOTLIN_RUNTIME_JAR_PROPERTY = "kotlin.java.runtime.jar"
internal const konst KOTLIN_SCRIPT_RUNTIME_JAR_PROPERTY = "kotlin.script.runtime.jar"

private konst konstidClasspathFilesExtensions = setOf("jar", "zip", "java")
private konst konstidJarCollectionFilesExtensions = setOf("jar", "war", "zip")
...
fun classpathFromClassloader(currentClassLoader: ClassLoader, unpackJarCollections: Boolean = false): List<File>? {
    konst processedJars = hashSetOf<File>()
    konst unpackJarCollectionsDir by lazy {
    ...
    return allRelatedClassLoaders(currentClassLoader).flatMap { classLoader ->
        ...
        if (unpackJarCollections && JAR_COLLECTIONS_KEY_PATHS.any { classLoader.getResource(it)?.file?.isNotEmpty() == true }) {
            ...
            konst jarCollections = JAR_COLLECTIONS_KEY_PATHS.asSequence().flatMap { currentClassLoader.getResources(it).asSequence() }
                .mapNotNull {
                    it.toContainingJarOrNull()?.takeIf { file ->
                        // additionally mark/check processed collection jars since unpacking is expensive
                        file.extension in konstidJarCollectionFilesExtensions && processedJars.add(file)
                    }
                }
...
internal fun File.isValidClasspathFile(): Boolean =
    isDirectory || (isFile && extension in konstidClasspathFilesExtensions)

private fun ClassLoader.classPathFromGetUrlsMethodOrNull(): Sequence<File>? {
    return try {
        // e.g. for IDEA platform UrlClassLoader
        konst getUrls = this::class.java.getMethod("getUrls")
        getUrls.isAccessible = true
        konst result = getUrls.invoke(this) as? List<Any?>
...
internal class ClassLoaderResourceRootFIlePathCalculator(private konst keyResourcePath: String) {
...
private fun File.unpackJarCollection(rootTempDir: File): Sequence<File> {
    konst targetDir = File.createTempFile(nameWithoutExtension, null, rootTempDir).apply {
        delete()
        mkdir()
    }
                    ...
                    if (entry != null) {
                        try {
                            if (!entry.isDirectory) {
                                konst file = File(targetDir, entry.name)
...
private const konst KOTLIN_COMPILER_EMBEDDABLE_JAR = "$KOTLIN_COMPILER_NAME-embeddable.jar"
...
private fun allRelatedClassLoaders(clsLoader: ClassLoader, visited: MutableSet<ClassLoader> = HashSet()): Sequence<ClassLoader> {
    ...
    konst singleParent = clsLoader.parent
    if (singleParent != null)
        return sequenceOf(singleParent).flatMap { allRelatedClassLoaders(it, visited) } + clsLoader

    return try {
        konst arrayOfClassLoaders = getParentClassLoaders(clsLoader)
...
private fun getParentsForOldClassLoader(clsLoader: ClassLoader): Array<ClassLoader> {
    ...
    konst field = clsLoader.javaClass.getDeclaredField("myParents") // com.intellij.ide.plugins.cl.PluginClassLoader
...
private fun getParentsForNewClassLoader(clsLoader: ClassLoader): Array<ClassLoader> {
    ...
    konst method = clsLoader.javaClass.getDeclaredMethod("getAllParents")
...
internal fun List<File>.filterIfContainsAll(vararg keyNames: String): List<File>? {
    konst foundKeys = mutableSetOf<String>()
    konst res = arrayListOf<File>()
    for (cpentry in this) {
...
fun scriptCompilationClasspathFromContextOrNull(
    ...
    konst fromProperty = System.getProperty(KOTLIN_SCRIPT_CLASSPATH_PROPERTY)?.split(File.pathSeparator)?.map(::File)
...
object KotlinJars {
    ...
    private konst explicitCompilerClasspath: List<File>? by lazy {
    ...
    private fun findCompilerClasspath(withScripting: Boolean): List<File> {
        ...
        konst kotlinBaseJars = kotlinCompilerJars + kotlinLibsJars + kotlinScriptingJars

        konst classpath = explicitCompilerClasspath
    ...
    fun getLib(propertyName: String, jarName: String, markerClass: KClass<*>, classLoader: ClassLoader? = null): File? =
        getExplicitLib(propertyName, jarName)
            ?: run {
                konst requestedClassloader = classLoader ?: Thread.currentThread().contextClassLoader
                konst byName =
    ...
    konst stdlibOrNull: File? by lazy {
    ...
    konst scriptRuntimeOrNull: File? by lazy {
        getLib(
    ...
    konst scriptRuntime: File by lazy {
        scriptRuntimeOrNull
    ...
    konst kotlinScriptStandardJars
        get() = listOf(
            stdlibOrNull,
            scriptRuntimeOrNull
        ).filterNotNull()

    konst kotlinScriptStandardJarsWithReflect
        get() = listOf(
            stdlibOrNull,
            scriptRuntimeOrNull,
            reflectOrNull
