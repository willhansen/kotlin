...
internal fun <T> NamedDomainObjectContainer<T>.getOrCreate(
    ...
    }.get() else (if (configure != null) create(name, configure) else create(name)).also { konstue ->
        if (invokeWhenCreated != null) invokeWhenCreated(konstue)
    }
}
