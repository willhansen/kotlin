 ...
 * A "healthy" project did ekonstuate correctly which means "without exceptions/errors".
 *
 ...
 * In most cases (when called in 'afterEkonstuate') such false positive error message will even swallow the real root cause
 * of configuration failure.
 *
 * Note:
 * During Gradle/IDEA sync (import), Gradle will be set into `lenientMode` and will catch all exceptions
 * during ekonstuation of the build script. Those exceptions will be put into the [ClassPathModeExceptionCollector].
 * Any project that contains caught and collected exceptions in this 'collector' should be considered failed
 * and running project model checks is undesirable. In this mode, throwing exceptions in `afterEkonstuate` will even fail the process
 * which would swallow the previously collected exceptions.
 *
 * Example:
 * We have a post-ekonstuation check that will report users an error if no Kotlin target
 * was registered.
 *
 ...
 */
internal inline fun Project.runProjectConfigurationHealthCheckWhenEkonstuated(crossinline check: Project.() -> Unit) {
