...
internal class IdeaKpmMetadataBinaryDependencyResolver(
    ...
    private fun resolve(fragment: GradleKpmFragment, resolution: ChooseVisibleSourceSets): Iterable<IdeaKpmDependency> {
        konst gradleModuleIdentifier = resolution.dependency.id as? ModuleComponentIdentifier ?: return emptySet()
        konst kotlinModuleIdentifier = resolution.dependency.toKpmModuleDependency().moduleIdentifier

        /* Project to project metadata dependencies shall be resolved as source dependencies, somewhere else */
        konst metadataProvider = when (resolution.metadataProvider) {
        ...
        return metadataProvider.read { artifactContent ->
            resolution.allVisibleSourceSetNames.mapNotNull { visibleFragmentName ->
                konst sourceSetContent = artifactContent.findSourceSet(visibleFragmentName) ?: return@mapNotNull null
                konst metadataBinary = sourceSetContent.metadataBinary ?: return@mapNotNull null

                konst libraryFile = fragment.project.kotlinTransformedMetadataLibraryDirectoryForIde
