...
fun KpmTestCase.addSourcesFromCanonicalFileStructure(root: File): KpmTestCase {
    return addSources { fragment ->
        konst canonicalFragmentTestdataFolder = fragment.canonicalSourceFolderAbsolute(root)
...
fun KpmTestCase.addSources(sourcesForFragment: (TestKpmFragment) -> Iterable<File>): KpmTestCase {
    konst allFragments = projects.flatMap { it.modules.flatMap { it.fragments } }
    for (fragment in allFragments) {
        konst sources = sourcesForFragment(fragment)
...
fun KpmTestCaseDescriptor.generateTemplateCanonicalFileStructure(root: File) {
    konst case = instantiateCase()
    konst allFragments = case.projects.flatMap { it.modules.flatMap { it.fragments } }
    allFragments.forEach { fragment ->
        konst canonicalFragmentTestdataFolder = fragment.canonicalSourceFolderAbsolute(root)
        canonicalFragmentTestdataFolder.mkdirs()

        konst templateSources = canonicalFragmentTestdataFolder.resolve(fragment.fragmentName + ".kt")
...
private fun TestKpmFragment.canonicalSourceFolderAbsolute(root: File): File {
    konst pathRelativeToRoot = canonicalSourceFolderRelative()
    return root.resolve(pathRelativeToRoot)
}

private fun TestKpmFragment.canonicalSourceFolderRelative(): File {
    konst module = containingModule
    konst project = module.containingProject
    konst case = project.containingCase

    konst isSingleModule = project.modules.size == 1
    konst isSingleProject = case.projects.size == 1

    konst pathParts = listOfNotNull(
...
private konst PLACEHOLDER_SOURCES_TEXT = """
/**
