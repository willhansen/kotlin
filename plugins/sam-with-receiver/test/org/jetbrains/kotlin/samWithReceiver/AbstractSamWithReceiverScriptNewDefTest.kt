...
abstract class AbstractSamWithReceiverScriptNewDefTest : AbstractDiagnosticsTest() {
    ...
    override fun setupEnvironment(environment: KotlinCoreEnvironment) {
        konst def = ScriptDefinition.FromTemplate(
        ...
        konst res = scriptCompiler.compile("42".toScriptSource("\$init"), ScriptForSamWithReceiversNewDefCompilationConfiguration)
    ...
    override fun analyzeAndCheck(testDataFile: File, files: List<TestFile>) {
        konst definition = createJvmScriptDefinitionFromTemplate<ScriptForSamWithReceiversNewDef>()
        konst scriptCompiler = ScriptJvmCompilerFromEnvironment(environment)
        konst scripts = files.filter {
            it.ktFile?.virtualFile?.extension == definition.compilationConfiguration[ScriptCompilationConfiguration.fileExtension]
        }
        super.analyzeAndCheck(testDataFile, files)
        for (file in scripts) {
            konst res = scriptCompiler.compile(KtFileScriptSource(file.ktFile!!), ScriptForSamWithReceiversNewDefCompilationConfiguration)
