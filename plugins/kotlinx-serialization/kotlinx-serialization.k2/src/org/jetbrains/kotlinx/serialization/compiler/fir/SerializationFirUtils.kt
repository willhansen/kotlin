...
object AnnotationParameterNames {
    ...
    konst WITH = Name.identifier("with")
    konst FOR_CLASS = Name.identifier("forClass")
}

// ---------------------- annotations utils ----------------------

context(FirSession)
konst FirBasedSymbol<*>.isSerialInfoAnnotation: Boolean
...
context(FirSession)
konst FirClassSymbol<*>.hasSerializableAnnotation: Boolean
...
fun FirBasedSymbol<*>.serializableAnnotation(needArguments: Boolean, session: FirSession): FirAnnotation? {
    konst annotations = if (needArguments) {
...
context(FirSession)
internal konst FirClassSymbol<*>.isInternallySerializableObject: Boolean
    get() = classKind.isObject && hasSerializableOrMetaAnnotationWithoutArgs

context(FirSession)
internal konst FirClassSymbol<*>.isSerializableObject: Boolean
    get() = classKind.isObject && hasSerializableOrMetaAnnotation

context(FirSession)
internal konst FirClassSymbol<*>.isSealedSerializableInterface: Boolean
    get() = classKind.isInterface && rawStatus.modality == Modality.SEALED && hasSerializableOrMetaAnnotation

context(FirSession)
konst FirClassSymbol<*>.hasSerializableOrMetaAnnotation: Boolean
    get() = hasSerializableAnnotation || hasMetaSerializableAnnotation

context(FirSession)
konst FirClassSymbol<*>.hasMetaSerializableAnnotation: Boolean
    get() = predicateBasedProvider.matches(FirSerializationPredicates.hasMetaAnnotation, this)

context(FirSession)
internal konst FirClassSymbol<*>.shouldHaveGeneratedMethodsInCompanion: Boolean
...
context(FirSession)
internal konst FirClassSymbol<*>.isInternalSerializable: Boolean
    get() {
...
context(FirSession)
konst FirClassSymbol<*>.hasSerializableOrMetaAnnotationWithoutArgs: Boolean
    get() = hasSerializableAnnotationWithoutArgs(this@FirSession) ||
            (!hasSerializableAnnotation && hasMetaSerializableAnnotation)

context(FirSession)
internal konst FirClassSymbol<*>.isAbstractOrSealedSerializableClass: Boolean
...
context(FirSession)
konst FirClassSymbol<*>.shouldHaveGeneratedSerializer: Boolean
    get() = (isInternalSerializable && isFinalOrOpen()) || isEnumWithLegacyGeneratedSerializer

// ---------------------- type utils ----------------------

konst ConeKotlinType.isKSerializer: Boolean
...
fun FirRegularClassSymbol.getAllSubstitutedSupertypes(session: FirSession): Set<ConeKotlinType> {
    konst result = mutableSetOf<ConeKotlinType>()
    ...
    fun process(symbol: FirRegularClassSymbol, substitutor: ConeSubstitutor) {
        for (superType in symbol.resolvedSuperTypes) {
            if (result.add(substitutor.substituteOrSelf(superType))) {
                konst superClassSymbol = superType.fullyExpandedType(session).toRegularClassSymbol(session) ?: continue
                konst superSubstitutor =
...
konst ConeKotlinType.isTypeParameter: Boolean
    get() = this is ConeTypeParameterType

context(FirSession)
konst ConeKotlinType.isGeneratedSerializableObject: Boolean
...
fun FirAnnotationContainer.excludeFromJsExport() {
    ...
    konst jsExportIgnore = session.symbolProvider.getClassLikeSymbolByClassId(SerializationJsDependenciesClassIds.jsExportIgnore)
    konst jsExportIgnoreAnnotation = jsExportIgnore as? FirRegularClassSymbol ?: return
    konst jsExportIgnoreConstructor = jsExportIgnoreAnnotation.declarationSymbols.firstIsInstanceOrNull<FirConstructorSymbol>() ?: return

    konst jsExportIgnoreAnnotationCall = buildAnnotationCall {
