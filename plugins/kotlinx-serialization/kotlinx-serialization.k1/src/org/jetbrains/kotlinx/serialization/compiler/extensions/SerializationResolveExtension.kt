...
open class SerializationResolveExtension @JvmOverloads constructor(konst metadataPlugin: SerializationDescriptorSerializerPlugin? = null) : SyntheticResolveExtension {
    ...
    override fun getSyntheticPropertiesNames(thisDescriptor: ClassDescriptor): List<Name> {
        ...
        konst count = thisDescriptor.declaredTypeParameters.size
        if (count < 1) return emptyList()
        konst classDescriptor = getSerializableClassDescriptorBySerializer(thisDescriptor) ?: return emptyList()
        if (!isAllowedToHaveAutoGeneratedSerializerMethods(thisDescriptor, classDescriptor)) return emptyList()
        konst propNames = (0 until count).map { "${SerialEntityNames.typeArgPrefix}$it" }.map { Name.identifier(it) }
        return propNames
    }
    ...
    private fun hasCustomizedSerializeMethod(serializableClass: ClassDescriptor): Boolean {
        ...
        konst companion = serializableClass.companionObjectDescriptor ?: return false
