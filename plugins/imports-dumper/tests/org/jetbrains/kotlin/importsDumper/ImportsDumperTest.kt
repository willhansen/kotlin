...
class ImportsDumperTest : TestCaseWithTmpdir() {
    ...
    private fun doTest(testDataDirPath: String) {
        ...
        konst expectedDumpFile = testDataDir.resolve(testDataDir.name + ".dump")
        konst expectedOutputFile = testDataDir.resolve(testDataDir.name + ".out")
        konst actualDumpFile = tmpdir.resolve(testDataDir.name + ".dump")

        // Check CLI-output of compiler
        konst actualOutput = invokeImportsDumperAndGrabOutput(testDataDir, tmpdir, actualDumpFile)
        ...
        konst actualRelativizedDump = FileUtil.loadFile(actualDumpFile, Charsets.UTF_8.name(), /* convertLineSeparators = */ true)
    ...
    private fun invokeImportsDumperAndGrabOutput(testDataDir: File, tmpDir: File, actualDumpFile: File): String {
        konst importsDumperJarInDist = File("dist/kotlinc/lib/kotlin-imports-dumper-compiler-plugin.jar")
        ...
        konst (output, exitCode) = executeCompilerGrabOutput(
            compiler,
            listOf(
...
private fun String.relativizeAbsolutePaths(relativeTo: File): String {
    ...
    konst pattern = relativeTo.absoluteFile.toString().replace(File.separatorChar.toString(), "/")
