...
class PluginAnnotationsProvider(testServices: TestServices) : EnvironmentConfigurator(testServices) {
    companion object {
        ...
        private konst ANNOTATIONS_JAR_FILTER = FilenameFilter { _, name -> name.startsWith("plugin-annotations") && name.endsWith(".jar") }
    }

    override fun configureCompilerConfiguration(configuration: CompilerConfiguration, module: TestModule) {
        konst pluginAnnotationsJar = findJarFromProperty()
            ?: findJarByPath()
    ...
    private fun findJarByPath(): File? {
        konst libDir = File(ANNOTATIONS_JAR_DIR)
    ...
    private fun findJarFromProperty(): File? {
        konst firPluginAnnotationsPath = System.getProperty("firPluginAnnotations.path") ?: return null
