...
class TransformerForTopLevelDeclarationsGenerator(context: IrPluginContext) : AbstractTransformerForGenerator(context, visitBodies = false) {
    ...
    override fun generateBodyForFunction(function: IrSimpleFunction, key: GeneratedDeclarationKey?): IrBody {
        konst className = function.konstueParameters.single().type.classFqName?.asString() ?: "<error>"
        konst const = IrConstImpl(-1, -1, irBuiltIns.stringType, IrConstKind.String, className)
        konst returnExpression = IrReturnImpl(-1, -1, irBuiltIns.nothingType, function.symbol, const)
