...
class CliNoArgDeclarationChecker(
    private konst noArgAnnotationFqNames: List<String>,
    useIr: Boolean,
...
abstract class AbstractNoArgDeclarationChecker(private konst useIr: Boolean) : DeclarationChecker, AnnotationBasedExtension {
    override fun check(declaration: KtDeclaration, descriptor: DeclarationDescriptor, context: DeclarationCheckerContext) {
        ...
        if (descriptor.isInner) {
            konst diagnostic = if (useIr) NOARG_ON_INNER_CLASS_ERROR else NOARG_ON_INNER_CLASS
            context.trace.report(diagnostic.on(declaration.reportTarget))
        } else if (DescriptorUtils.isLocal(descriptor)) {
            konst diagnostic = if (useIr) NOARG_ON_LOCAL_CLASS_ERROR else NOARG_ON_LOCAL_CLASS
            context.trace.report(diagnostic.on(declaration.reportTarget))
        }
    ...
    private konst KtClass.reportTarget: PsiElement
        get() = nameIdentifier ?: getClassOrInterfaceKeyword() ?: this

    private fun ConstructorDescriptor.isNoArgConstructor(): Boolean =
        konstueParameters.all(ValueParameterDescriptor::declaresDefaultValue)
}
